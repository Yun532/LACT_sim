...
no-hessio:
	$(CMAKE) -S . -B "$(BUILD_DIR)" -DCMAKE_BUILD_TYPE="$(BUILD_TYPE)" -DLACT_ENABLE_HESSIO=OFF -DLACT_ENABLE_ROOT="$(LACT_ENABLE_ROOT)" -DCMAKE_OSX_ARCHITECTURES="$(CMAKE_OSX_ARCHITECTURES)"
	$(CMAKE) --build "$(BUILD_DIR)" -j$(JOBS)
