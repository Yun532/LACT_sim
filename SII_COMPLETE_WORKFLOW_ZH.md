...
本文原有图 1–7 来自解析长曝光 notebook；新增图 8、8a 和 9 来自
`intensity-interferometry-waveform-gls` 分支的波形 GLS notebook 从头重算。
两条路径的区别会在第 8 节明确列出，不能混用其误差数值。全部随机实验使用固定种子，图片不是手工示意图。
...
新方法中每个20 min原始测量的 $\sigma_P=0.19225$。当前数据的 $P$ 从接近0延伸到0.985；当 $P\simeq1$ 时单点 SNR 约为5.20。8928个测量的中位单点 SNR 为1.685，约65.0%的点大于1，约23.7%的点大于3。它比旧解析路径更保守，但仍有大量不同方向和尺度的联合约束。

重建也不是从一个点作图，而是同时拟合所有测量。以 $120\ \mathrm{M}\lambda$ 网格合并后，8928 个测量形成1021个实际UV格；再加入由总流量归一化给出的零基线 $P(0,0)=1$，优化器共使用1022个约束点。一个UV格的测量重复数中位数为6，因此典型合并误差由0.19225降到约0.0785。源的信号还具有跨UV平面连续、成体系的条纹结构，非负性、有限支撑、总流量归一化和平滑正则会排除大量纯噪声图像。
...
不过当前重建仍偏乐观，原因包括：源很亮（$m_{\rm AB}=2$）；缺失的增益漂移、时钟误差、附加电子噪声和跨望远镜共享噪声均设为0；波形标定只建立了单基线内部101个滞后的协方差，尚未建立共享望远镜导致的跨基线协方差；重建还使用非负、有限视场和弱平滑先验。因此图9是在当前已知输入下的闭合结果，不是对真实LACT六小时必然成像质量的保证。
...
在 UV 几何已接近饱和后，继续重复观测仍会降低误差，但收益是平方根而非线性。例如相同 UV 网格在5夜共30 h和10夜共60 h后，重复测量逆方差合并的单点误差分别约为0.0860和0.0608；6 h单夜中一个20 min原始波形GLS测量的误差仍为0.19225。这里降低的是重复采样合并后的误差，不是把每个原始20 min点改写成更高精度。
...
这些数字是“单基线、单位 $P$、假设2 nm恒星通道、main宽带NSB压力值和理想匹配处理”条件下的检测门槛，不是最终硬件星等极限，也不是复杂双星可靠成像极限。新方法目前只对 $m_{\rm AB}=2$ 完成波形标定，得到20 min、$P=1$ 的 SNR 为5.20。因为改变星等会同时改变波形方差、SPE重叠和散粒噪声占比，不能只把这一个 $\sigma_P$ 按光子率缩放；应对每个星等重新生成 $\boldsymbol k$ 和 $\boldsymbol\Sigma$，再用单星零假设与双星备择假设做多随机种子检出率研究。多光谱通道若彼此统计独立，总SNR理想情况下按 $\sqrt{N_{\rm ch}}$ 增长；最终灵敏度仍必须让恒星和NSB共用同一个实测总响应 $T(\lambda)$。
...
| 完整峰 GLS 标定 | 波形 Monte Carlo | 101个滞后；1024条零信号和256条增强注入记录；20 min $\sigma_P=0.192247$ |
...
因此，当前时间响应、SPE、短—长曝光 GLS 统计量已经在 $m_{\rm AB}=2$ 参考场景闭合，但光谱部分仍是“2 nm 恒星通道 + main 宽带 NSB 压力值”的保守混合场景，不是最终自洽通带。缺失系统误差设0可避免虚构参数；代码在波形GLS模式下若发现这些RMS被设为非零，会明确报错，而不是悄悄忽略。形成硬件灵敏度结论前，必须用同一个总响应 $T(\lambda)$ 同时计算 $r_\star$、$r_{\rm NSB}$、$s_{\rm spec}$ 和 $\tau_{\rm eff}$，并对目标星等重新标定波形统计量。
...
`waveform_gls_validation.ipynb` 把“仪器读取 → 短波形标定 → 留出验证 → 长曝光协方差缩放 → 32台UV数据 → 重建”完整串联并输出图8、8a和9。原 `lact_sii_paper_simulation.ipynb` 仍保留解析上限、不同星等和几何扫描；在这些扫描迁移到逐场景波形标定前，不能把其中解析灵敏度称为新方法结果。
