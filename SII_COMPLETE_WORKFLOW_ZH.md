...
其中 $t_s$ 是该段中点。程序用 9 个等间隔段内时刻作数值积分；表中的 $(u,v)$ 仍用段中点坐标代表这一小段 UV 轨迹。当前双星案例中，段平均值与段中点值的最大差约为 0.0069，小于单点统计误差 0.2348，但保留段平均可避免在更长分段、更长基线或更复杂源上产生时间平均偏差。
...
程序不是把 SNR 送入重建，也不是把 $\overline P$ 原样送入重建；它先抽取

$$
\widehat P=\overline P+\epsilon,
$$

然后重建器读取 $(u,v,\widehat P,\sigma_P)$。这里的 $\sigma_P$ 与第 8.2 节完全相同：它包含恒星、NSB 和暗计数的散粒噪声，$\tau_{\rm eff}$，由光学时间核、SPE、采样和已知电子噪声得到的 $B_{\rm eff}$，以及单 PE 电荷涨落 $F_{\rm EN}$；已启用的增益、时钟和零点系统误差另行进入 $A$、$G$、$z$ 或总误差。当前缺失的后几项仍为 0。
...
**图 7。** 左：每条基线每个 20 min 段沿 UV 轨迹平均后的无噪声期望 $\overline P$。中：从同一批期望值各抽取一次统计误差后，真实交给重建的 $\widehat P$。右：同一次 realization 中 $\widehat P$ 相对 $\overline P$ 的散布。绘图色标限制在 0 到 1，超出范围的噪声值只在显示时颜色饱和，CSV 数据没有裁剪。正负基线使用相同测量值和显著度。
...
均值为 0.0158、样本标准差为 0.9923，和注入的标准正态误差一致；约 $15.3\%$ 的 $\widehat P$ 为负值。这个检查直接证明图中的模拟 UV 数据不是 $P$ 真值本身，而是具有第 8.2 节预测方差的一次随机观测。
...
因为目标函数是非凸的，程序使用多个初值并比较正向残差。图 8 对应的默认 6 小时 realization 中，加权正向 RMSE 为 0.0829；共同分辨率卷积后与真值的相关系数为 0.9651。恢复分离角为 0.2025 mas，真值为 0.2000 mas；恢复位置角为 $39.8^\circ$，真值为 $35.0^\circ$；恢复峰值比为 0.789，真值流量比为 0.55。后两项的偏差说明“能看出是双星”不等于位置角和测光都已准确。
