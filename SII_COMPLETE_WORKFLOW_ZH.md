...
GLS 方法验证；图 10–20 及参数影响表来自当前完整论文报告 notebook
...
个6 h噪声实现中取严格嵌套的中心时段。20 min结果出现多个同等级伪峰，图像
相关系数仅0.5123；2 h和6 h逐渐形成稳定双峰，图像相关系数分别提高到
0.8285和0.8820。回收分离角依次为0.1933、0.2046和0.2191 mas，输入真值
为0.20 mas；这也说明单独看“两峰间距”会把噪声峰误认为源。加权可见度
RMSE依次为2.4119、1.7293和1.2064。该图
说明延长观测可稳定当前这一随机实现，但不能据此宣称“4等星在6 h必然可成像”：正式灵敏度结论仍需多随机
...
**图 20。** 同一个2等星凌日源在1、4和10个独立夜晚累计后的无相位重建，
对应总积分6、24和60 h。第一夜与图16使用完全相同的测量和重建；后续夜晚
使用波形GLS给出的单夜$\sigma_P$生成独立统计噪声，并按夜累计平均。每个UV点
的$\sigma_P$依次为0.1742、0.0871和0.0551，加权可见度RMSE依次为0.0583、
0.0291和0.0184，图像相关系数由0.8415提高到0.8945和0.9283。恒星圆盘与
凌日暗区随积分时间变清楚，但60 h图仍存在额外暗结构；这表明延长曝光降低了
随机误差，却没有消除模平方数据缺少傅里叶相位带来的非唯一性。本比较假设
...
`sii_complete_waveform_report.ipynb` 是当前主入口，完整生成图10–20和参数
