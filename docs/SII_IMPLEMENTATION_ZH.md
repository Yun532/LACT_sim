...
图1第三幅是另一种理论输出：它画随皮秒时延变化的光学强度相关，而不是上述曝光平均功率。`sii_walkthrough.theoretical_scene` 在399–401 nm的4097个波长节点分别算所选基线的复可见度，再用 `density=response/wavelength` 与 `phase=exp(-2j*pi*(frequency-frequency.mean())*lag)` 做梯形积分并除以光谱积分。`field_pair` 保存的是去掉共同载频后的归一场相关，并非随机电场；`field` 是将可见度置1的零基线对照。随后 `g2_pair=1+0.5*abs(field_pair)**2`，零基线同理。光谱积分、变量含义与热光假设的完整说明见[物理说明第2.2–2.5节](SII_PHYSICS_ZH.md#figure-one-chain)。

因此图1没有先生成一条电场或光电子记录；其输出是801个时延点上的理论总体平均。代码将它写入 `walkthrough_ideal_hbt.csv`，零基线与两镜零时延值分别为1.5和1.4164554。后续事件生成要用相关峰的时间积分，才得到平方可见度的光谱平均；这个平均与图1先积分复可见度再取模平方的顺序不同，物理说明附录C用Parseval关系连接两者。
