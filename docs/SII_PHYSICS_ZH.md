...
第$i$台镜中第$k$个光电子的时间为$t_{ik}$，归一化电荷为$Q_{ik}$；SPE电压模板记为$h_{\rm SPE}(t)$，单位mV。采样时刻$t_n=n\Delta t$，噪声电压为$e_i[n]$。量化前波形为
...
| 波形统计 | 散粒噪声占优的弱HBT二阶近似 | 高简并度或多镜相关研究需要完整热光统计和镜间联合波形验证 |
