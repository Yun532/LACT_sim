...
主Notebook第4.1节实际运行联合观测程序，并生成本节的图表。核宽对照进一步将16、32、64、128、256、512点半宽的输出裁到**同一19.352 μs时间段**，从而排除曝光长度不同造成的变化。固定第7节所述的解析GLS投影，并除以已知注入倍率和功率真值，得到每种核的无量纲响应。比较前不给各核单独除以新标定增益，否则响应差会被标定吸收。
...
基线12的响应差随半宽增加为：16点−0.02332、64点−0.004947、128点−0.001780、256点−0.0006444；对应256点配对均值标准误为0.0001413。三条基线256至512点的逐项响应差及其正态近似95%范围均位于约0.1%量级以内。[完整表](../validation/sii_observation/kernel_convergence.csv)同时保留各核的噪声标准差比。这个结论局限于给定SPE、三个分数延迟及固定投影，不能推广为全部采样相位或连续模拟响应的0.1%准确度。默认16点处理仍使用其专属标定；需要更小核截断误差时可选择更宽核，并匹配相应响应和噪声。
...
若缩放近似适用，$q_T$应与1相容。本次直接生成96 μs记录，公共裁剪后使用95.320 μs，与短记录的23.320 μs比较；没有把四条短记录拼起来或直接除以2制造结果。同一响应增益在标准差比中抵消。[曝光对照表](../validation/sii_observation/exposure_scaling.csv)给出三条基线的比值1.041、1.014、0.976，95%区间分别为[0.942, 1.151]、[0.917, 1.121]、[0.883, 1.079]，均包含1。即使在这一范围闭合，也只支持固定仪器状态下的微秒记录缩放，不能代替小时级时变时延、背景、增益及天气状态的检查。
