...
阅读导航：正文第 1–6 节解释源、通带、阵列与电压生成；第 7–9 节解释相关、GLS 与整夜观测；第 10–13 节解释模型、反演、性能和规格书；[第14节](#instrument-design)解释仪器参数怎样影响信号、SNR与天体约束，并给实际设计对照。关键推导集中于[附录 A–K](#appendix-a)，[附录 L](#appendix-l)列出公式与函数、结果数组的对应。
...
设 $Q_\nu(\nu)$ 是通带后有效光子谱，单位s$^{-1}$ Hz$^{-1}$；$V_{ij}(\nu)$ 是该频率下的空间可见度。两镜响应相同、源平稳时，实际计算的关系为
...
附录A构造的场是高斯变量的线性组合，所以仍为联合复高斯场。现在取 $E_i=E_i(t+\tau)$、$E_j=E_j(t)$，这里只为简化写法暂时省略时间自变量。令 $\Gamma_{ij}=\langle E_iE_j^*\rangle$，$\mu_i=\langle J_i\rangle$，分别为未归一场相关和平均强度，单位均为s$^{-1}$。
...
一般高斯四阶矩有三种两两配对：$(E_i,E_i^*)(E_j,E_j^*)$、$(E_i,E_j)(E_i^*,E_j^*)$、$(E_i,E_j^*)(E_i^*,E_j)$。中间一种因为圆对称的无共轭相关为零而消失，另两种分别为 $\mu_i\mu_j$ 与 $|\Gamma_{ij}|^2$。所以不是凭经验把可见度平方加到1上，而是先完成这个四阶矩平均。
