...
CORSIKA 页的 LACT u/v 也不是由网页根据 photon bunch 的方向反算。相机几何直接来自同一个 ROOT 的 `camera_pixels.x_m/y_m/size_m`，其中 `x_m=u`、`y_m=v`；所选望远镜的信号来自 `observations.image_cherenkov_pe`，按 `pixel_id` 与这 1616 个几何像素连接。生成时会把 ROOT 几何与相机配置逐像素比较。
