...
默认 `measured_spe_4ns.cfg` 不包含微单元恢复、基线涨落、电子学噪声、SiPM
本征时间抖动、串扰、后脉冲、暗计数。恢复链路可由
`measured_spe_recovery_10ns_1p6ns.cfg` 单独开启；其中 10 ns 是待标定假设。
NSB仍是独立的primary-PE来源，可单独开关；纯切伦科夫验证默认关闭。
