...
- 单位：米。
- 坐标系：望远镜本地坐标系。
- 默认镜片布局中，镜面附近约在 `z=-16 m`，白板/相机附近约在
  `z=-8 m`。
...
程序由调用位置明确区分入射段和反射段，不再根据局部 `dir.z` 的正负猜测。
这对低仰角以及记录面位于镜面另一侧的光线是必要的。
...
- `camera_body` 和 `camera_support_gap_box` 只参与入射方向相关的遮挡，不参与
  镜面反射到白板/相机的遮挡。
- CORSIKA 入射遮挡从镜面沿光子来向反查，不把 2D 记录点误当成光子产生点。
- 入射段一旦遮挡，该光子立即结束，不再重复统计反射段遮挡。
