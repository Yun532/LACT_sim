/* ============================================================================
   ...
   Copyright (C) 2000, 2003, 2006, 2007, 2008, 2009, 2010, 2014, 2015, 2016, 
                 2017, 2018, 2019, 2021, 2022, 2023, 2024, 2025  Konrad Bernloehr
...
#ifdef REQUIRE_FILEOPEN_PREFIX
# define STATIC_RFP static
STATIC_RFP void initpath(const char *default_path);
STATIC_RFP void initexepath(const char *default_path);
STATIC_RFP void listpath (char *buffer, size_t bufsize);
STATIC_RFP void addpath(const char *name);
STATIC_RFP void addexepath(const char *name);
STATIC_RFP struct incpath *get_include_path(void);

STATIC_RFP void set_permissive_pipes(int p);
STATIC_RFP void enable_permissive_pipes(void);
STATIC_RFP void disable_permissive_pipes(void);
#else
# define STATIC_RFP
#endif
...
/* ----------------- fileopen_set_verbose -------------------- */
/**
 *  @short Dynamically set/unset verbose mode and report back 
 *         the previous setting, in case we want to restore it later-on.
 *
 *  Override at run-time what may have been initialized from the
 *  FILEOPEN_VERBOSE environment variable once. 
 */
...
/* ----------------- fileopen_get_verbose -------------------- */
/**
 *  @short Get current status of verbose mode without changing anything.
 */

int fileopen_get_verbose()
{
   return verbose;
}
...
/* ----------------- fileopen_get_errno ----------------- */
/**
 *  @short If we locally save encountered error codes, we can
 *         obtain the last known error.
 */

int fileopen_get_errno(void)
{
   if ( s_errno == 0 ) /* No error known from this module ? */
      return errno;    /* Then report what we have globally. */
   else
      return s_errno;  /* Otherwise report what fileopen got the last time around. */
}
#endif
...
static FILE *popenx(const char *fname, const char *mode)
{
   ...
   else
   {
      ...
      if ( errno != k )
         errno = k; /* Restore the error number if it got touched */
   }
   return f;
}
...
{
   set_permissive_pipes(p);
}


/** Enable the permissive execution of pipes. */

STATIC_RFP void enable_permissive_pipes()
{
   permissive_pipes = 1;
...
void fileopen_enable_permissive_pipes()
{
   enable_permissive_pipes();
}
...
STATIC_RFP void disable_permissive_pipes()
{
   permissive_pipes = 0;
}

void fileopen_disable_permissive_pipes()
{
   disable_permissive_pipes();
}
...
STATIC_RFP void initexepath (const char *default_exe_path)
{
   int ipos = 0;
   char pathname[PATH_MAX];
...
void fileopen_initexepath (const char *default_exe_path)
{
   initexepath(default_exe_path);
}
