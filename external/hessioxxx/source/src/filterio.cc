...
Copyright (C) 2014, 2023, 2024 Konrad Bernloehr (Konrad Bernl&ouml;hr)
...
Options:
  ...
  -q  Quiet mode, not even showing total blocks count (--quiet).
  -n  Not accepting any blocks of the listed types (--not).
      The default is to accept only the listed types.
  -A num  Activate output with block of type <num>.
      Same: --activate-with
  -a num  Stop after processing a block of type <num>.
      Same: --stop-after
  -b num  Stop before processing a block of type <num>.
      Same: --stop-before
...
void syntax(const char *prg)
{
   ...
   fprintf(stderr,"      Same: --statistics\n");
   fprintf(stderr,"  -v  Verbose output (--verbose).\n");
   fprintf(stderr,"  -q  Quiet mode, not even showing total blocks count (--quiet).\n");
   fprintf(stderr,"  -n  Not accepting any blocks of the listed types (--not).\n");
   fprintf(stderr,"      The default is to accept only the listed types.\n");
   fprintf(stderr,"  -A num  Activate output with block of type <num>.\n");
   fprintf(stderr,"      No input data before that gets written to output.\n");
   fprintf(stderr,"      Same: --activate-with\n");
   fprintf(stderr,"  -a num  Stop after processing a block of type <num>.\n");
   fprintf(stderr,"      Same: --stop-after\n");
   fprintf(stderr,"  -b num  Stop before processing a block of type <num>.\n");
   fprintf(stderr,"      Same: --stop-before\n");
   fprintf(stderr,"  -o fname  Output goes to given file instead of standard output.\n");
   fprintf(stderr,"As long as no block types are specified, all data gets accepted.\n");
   exit(1);
}
...
int main (int argc, char **argv)
{
   ...
   bool done = false, done_delayed = false, active = true;
   char *prg = argv[0];
   size_t nf = 0;
   size_t ni = 0, no = 0;
   vector<unsigned long> stop_before, stop_after;
   unsigned long activate_with = 0;
   ...
   for (int iarg=1; iarg<argc; iarg++)
   {
      ...
      if ( (string(argv[iarg]) == "-A" ||  string(argv[iarg]) == "--activate-with") && iarg+1<argc  )
      {
         activate_with = (unsigned long) atol(argv[++iarg]);
         continue;
      }
      if ( (string(argv[iarg]) == "-a" ||  string(argv[iarg]) == "--stop-after") && iarg+1<argc  )
      {
         stop_after.push_back((unsigned long) atol(argv[++iarg]));
         continue;
      }
      if ( (string(argv[iarg]) == "-b" ||  string(argv[iarg]) == "--stop-before") && iarg+1<argc  )
      {
         stop_before.push_back((unsigned long) atol(argv[++iarg]));
         continue;
      }
      if ( (string(argv[iarg]) == "-o" || string(argv[iarg]) == "--output") && iarg+1<argc )
      {
      ...
      // Use './3' if you really have a file named '3'.
      ...
      // For each input file loop over all of its data blocks
      for (;;)
      {
         // Find the next data block and read it into memory.
         if ( iobuf.Find() < 0 )
            break;
...
// cerr << "Read data block of type " << it << "\n";

         // Accumulate input data block statistics, if requested.
         if ( show_stats )
         {
         ...
         if ( !active ) // Waiting for the block type to activate?
         {
            if ( it == activate_with )
            {
// cerr << "Activate with data block of type " << it << "\n";
               active = true;
            }
            else
            {
// cerr << "Not active yet\n";
         ...
         // Stop output before the first of given type(s) of blocks?
         if ( stop_before.size() > 0 )
         {
            size_t n = stop_before.size();
            ...
            if ( done )
            {
// cerr << "Done before type " << it << "\n";
               break;
            }
         }

         // Stop output after the first of given type(s) of blocks?
         if ( stop_after.size() > 0 )
         {
            size_t n = stop_after.size();
            for ( size_t i=0; i<n; i++ )
            {
               if ( it == stop_after[i] )
               {
                  done_delayed = true;
// cerr << "Supposed to be done after type " << it << "\n";
                  break;
               }
            }
         }

// cerr << "Check if we are interested in this data block\n";
         if ( type_list.size() > 0 )
         {
            // Check if the block type is in our list.
            bool in_list = false;
            ...
            if ( negate ) // Do we want everything but blocks of those types?
               in_list = !in_list;
            if ( !in_list ) // Skip this block if it is not what we want.
            {
// cerr << "Not interested\n";
               continue;
            }
...
// cerr << "Writing the data block";
         // That is where the data goes to the output.
         if ( iobuf.Write() != 0 )
            break;
         no++;

         // Accumulate output data block statistics, if requested.
         if ( show_stats )
         {
         ...
         // If we should be done with this input file, we finish with it.
         if ( done_delayed )
            done = true;
         if ( done )
            break;
