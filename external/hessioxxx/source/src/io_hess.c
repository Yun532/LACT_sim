/* ============================================================================
   ...
   Copyright (C) 2000, 2001, ..., 2019, 2020, 2021, 2022, 2023, 2024, 2025  Konrad Bernloehr
 ...
 *  @date July 2000 (initial version) to 2025
 */
...
static void hs_check_env()
{
   ...
   if ( (s = getenv("PRINT_VERBOSE")) != NULL )
   {
      if ( ! isdigit(*s) )
         hs_verbose = 1;
      else
...
int write_simtel_runheader (IO_BUFFER *iobuf, RunHeader *rh)
{
   ...
   item_header.type = IO_TYPE_SIMTEL_RUNHEADER;  /* Data type */
...
int write_simtel_mcrunheader (IO_BUFFER *iobuf, MCRunHeader *mcrh)
{
   ...
   item_header.type = IO_TYPE_SIMTEL_MCRUNHEADER;  /* Data type */
   ...
   /* The following is limited to <= 20 by CORSIKA event header format,
      even if there are more than 20. See teloffset data blocks. */
...
int print_simtel_mcrunheader (IO_BUFFER *iobuf)
{
   ...
   int n_use = get_int32(iobuf);
   if ( n_use != 20 )
      printf("  Number of arrays per shower: %d\n",n_use);
   else
      printf("  Number of arrays per shower: %d (or more ?)\n",n_use);
   i = get_int32(iobuf);
   ...
   if ( item_header.version >= 3 )
   {
      ...
      if ( corsika_high_E_detail > 0 )
      {
         ...
         if ( m_high_E == 3 )
         {
            ...
            else if ( corsika_high_E_detail%100 == 4 )
               printf("  This is QGSJET-III.\n");
         ...
         {
            if ( corsika_high_E_detail%100 == 2 )
               printf("  This is Sibyll 2.1 or newer.\n");
            else if ( corsika_high_E_detail%100 == 1 )
...
int write_simtel_camsettings (IO_BUFFER *iobuf, CameraSettings *cs)
{
   ...
   item_header.type = IO_TYPE_SIMTEL_CAMSETTINGS;  /* Data type */
   // item_header.version = 2;  /* Version 2 has no camera rotation and no pixel inclination */
   // if ( cs->cam_rot != 0. )
   //    item_header.version = 3; /* Version 3 has camera rotation but  no pixel inclination */
   item_header.version = 4;  /* Need version 4 for pixel inclination */
   if ( cs->eff_flen != 0. )
      item_header.version = 5; /* Single effective focal length included in version 5 */
   if ( (cs->eff_flen_x != 0. && cs->eff_flen_x != cs->eff_flen) ||
        (cs->eff_flen_y != 0. && cs->eff_flen_y != cs->eff_flen) )
      item_header.version = 6; /* Version 6 can hint different scales in x and y */
   item_header.ident = cs->tel_id;
   put_item_begin(iobuf,&item_header);

   put_int32(cs->num_pixels,iobuf); /* Number of pixels always as int32 */
   put_real(cs->flen,iobuf);
   if ( item_header.version > 4 )
      put_real(cs->eff_flen,iobuf);
   if ( item_header.version > 5 )
   {
      put_real(cs->eff_flen_x,iobuf);
      put_real(cs->eff_flen_y,iobuf);
      put_real(cs->eff_flen_dx,iobuf);
      put_real(cs->eff_flen_dy,iobuf);
   }
...
int read_simtel_camsettings (IO_BUFFER *iobuf, CameraSettings *cs)
{
   ...
   cs->eff_flen = cs->eff_flen_x = cs->eff_flen_y = 0.;
   cs->eff_flen_dx = cs->eff_flen_dy = 0.;
   if ( item_header.version > 4 )
      cs->eff_flen = cs->eff_flen_x = cs->eff_flen_y = get_real(iobuf);
   if ( item_header.version > 5 )
   {
      cs->eff_flen_x = get_real(iobuf);
      cs->eff_flen_y = get_real(iobuf);
      cs->eff_flen_dx = get_real(iobuf);
      cs->eff_flen_dy = get_real(iobuf);
   }
...
int print_simtel_camsettings (IO_BUFFER *iobuf)
{
   ...
   if ( item_header.version > 5 )
   {
      double eff_flen_x = get_real(iobuf);
      double eff_flen_y = get_real(iobuf);
      double eff_flen_dx = get_real(iobuf);
      double eff_flen_dy = get_real(iobuf);
      printf("   eff_flen_x = %f m\n", eff_flen_x);
      printf("   eff_flen_y = %f m\n", eff_flen_y);
      printf("   eff_flen_dx = %f m (displaced in x)\n", eff_flen_dx);
      printf("   eff_flen_dy = %f m (displaced in y)\n", eff_flen_dy);
   }
   printf("   xpix = ");
...
int write_simtel_camorgan (IO_BUFFER *iobuf, CameraOrganisation *co)
{
   ...
   item_header.type = IO_TYPE_SIMTEL_CAMORGAN;  /* Data type */
   ...
   put_int32(co->num_pixels,iobuf);     /* Number of pixels always as int32 */
   put_int32(co->num_drawers,iobuf);    /* Number of drawers always as int32 */
   put_int32(co->num_gains,iobuf);
   if ( item_header.version >= 1 )
      put_int32(co->num_sectors,iobuf); /* Number of trigger sectors only with version >= 1 */
   if ( item_header.version <= 1 )
   {
      /* Despite the number of pixels and the number of drawers stored as int32,
         the version 0 would still fail in general with >= 32767 pixels or drawers. */
   ...
   {
      /* Support the full int32 range in pixels and drawers. */
   ...
   /* Version 1 also adds information on trigger sectors. */
   if ( item_header.version >= 1 )
   {
...
int read_simtel_camorgan (IO_BUFFER *iobuf, CameraOrganisation *co)
{
   ...
   if ( item_header.version <= 1 )
   {
      ...
      for (i=0; i<co->num_pixels; i++)
      {
         ...
         /* Fix for old bug in write_simtel_camorgan(): trailing '0' sectors. */
...
int write_simtel_pixelset (IO_BUFFER *iobuf, PixelSetting *ps)
{
   ...
   item_header.type = IO_TYPE_SIMTEL_PIXELSET;  /* Data type */
   if ( ps->time_slice == 0. )
      item_header.version = 0;             /* We can use the older format */
   else
      item_header.version = 1;             /* Version 1+ includes the number and width of readout/sum intervals */
   if ( ps->nrefshape > 0 && ps->lrefshape > 0 )
      item_header.version = 2;             /* Version 2+ can hold reference pulse shape */
   if ( ps->sum_offset != 0 )
      item_header.version = 3;             /* Version 3 can hold readout offset */
   ...
   put_int32(ps->num_pixels,iobuf);        /* Number of pixels always as int32 */
   ...
   if ( item_header.version >= 3 )
      put_int32(ps->sum_offset,iobuf);

   return put_item_end(iobuf,&item_header);
}

/* -------------------- read_simtel_pixelset ------------------- */
/**
...
int print_simtel_pixelset (IO_BUFFER *iobuf)
{
   ...
   if ( item_header.version >= 2 )
   {
      ...
      lr = get_scount(iobuf);
      rs = get_real(iobuf);
      printf("   Reference pulse shapes of %d (sub-) samples of %4.2f ns for %d gain%s.\n",
         lr, rs, nr, nr==1?"":"s");
      if ( hs_verbose )
      {
         for (n=0; n<nr; n++)
   ...
   if ( item_header.version >= 3 )
      printf("   Summation/readout starts %d time slices before trigger.\n", get_int32(iobuf));
   else
      printf("   Summation/readout offset is not known.\n");

   return get_item_end(iobuf,&item_header);
}

/* -------------------- write_simtel_pixeldis ------------------- */
/**
...
int write_simtel_pixeldis (IO_BUFFER *iobuf, PixelDisabled *pd)
{
   ...
   item_header.type = IO_TYPE_SIMTEL_PIXELDISABLE;  /* Data type */
...
int print_simtel_pixeldis (IO_BUFFER *iobuf)
{
   ...
   {
      printf("   Trigger disabled in %d pixels:", num_trig_disabled);
      for ( i=0; i<num_trig_disabled; i++)
      {
         int ipix = get_int32(iobuf);
         if ( i < hs_maxprt )
            printf(" %d", ipix);
         else if ( i == hs_maxprt )
            printf(" ...");
   ...
   {
      printf("   HV disabled in %d pixels:", num_HV_disabled);
...
int write_simtel_camsoftset (IO_BUFFER *iobuf, CameraSoftSet *cs)
{
   ...
   item_header.type = IO_TYPE_SIMTEL_CAMSOFTSET;  /* Data type */
   item_header.version = 0;             /* Version 0 (test) */
   item_header.ident = cs->tel_id;
   put_item_begin(iobuf,&item_header);
...
int write_simtel_trackset (IO_BUFFER *iobuf, TrackingSetup *ts)
{
   ...
   item_header.type = IO_TYPE_SIMTEL_TRACKSET;  /* Data type */
   item_header.version = 0;             /* Version 0 (test) */
...
int write_simtel_pointingcor (IO_BUFFER *iobuf, PointingCorrection *pc)
{
   ...
   item_header.type = IO_TYPE_SIMTEL_POINTINGCOR;  /* Data type */
   item_header.version = 0;             /* Version 0 (test) */
...
int write_simtel_centralevent (IO_BUFFER *iobuf, CentralEvent *ce)
{
   ...
   int ntt = 0, itel, itrg;
   int trg_type_any = 0;
   int max_trg_types = H_MAX_TRG_TYPES;

   if ( iobuf == (IO_BUFFER *) NULL || ce == NULL )
      return -1;

   if ( H_MAX_TRG_TYPES > 4 )
   {
      /* Combine all trigger type masks actually used. */
      for ( itel=0; itel<ce->num_teltrg; itel++ )
      {
         trg_type_any |= ce->teltrg_type_mask[itel];
      }
      /* Which is the highest bit actually allowed there? */
      /* Any bits used where not allowed, are ignored here. */
      if ( H_MAX_TRG_TYPES >= 8 && (trg_type_any & 0x80) )
         max_trg_types = 8;
      else if ( H_MAX_TRG_TYPES >= 7 && (trg_type_any & 0x40) )
         max_trg_types = 7;
      else if ( H_MAX_TRG_TYPES >= 6 && (trg_type_any & 0x20) )
         max_trg_types = 6;
      else if ( H_MAX_TRG_TYPES >= 5 && (trg_type_any & 0x10) )
         max_trg_types = 5;
      else
         max_trg_types = 4; /* Fallback to number guaranteed by older data block versions */
   }

   item_header.type = IO_TYPE_SIMTEL_CENTEVENT;  /* Data type */
   item_header.version = 2;             /* Version 2 or 3 */
   if ( ce->ls_comp > 0. )
      item_header.version = 3;
   if ( max_trg_types > 4 )
      item_header.version = 4;          /* Request a record of the number of trigger types to support */
   item_header.ident = ce->glob_count;
   put_item_begin(iobuf,&item_header);

   put_time_blob(&ce->cpu_time,iobuf);
   put_time_blob(&ce->gps_time,iobuf);
   put_int32(ce->teltrg_pattern,iobuf);  /* Bit pattern only useful with <= 31 telescopes */
   put_int32(ce->teldata_pattern,iobuf);

   if ( item_header.version <= 3 && H_MAX_TRG_TYPES > 4 )
      max_trg_types = 4; /* Never more than 4 trigger types unless using version 4+ */
   if ( item_header.version >= 4 )
      put_short(max_trg_types,iobuf); /* How many different trigger types to be supported when reading the data */

   if ( item_header.version >= 1 )
   {
      /* Version 1 adds telescope lists for trigger and data, for <= 32767 telescopes.
         Also adds a single trigger time per telescope. */
      put_short(ce->num_teltrg,iobuf); /* Number of telescopes that triggered */
      put_vector_of_int(ce->teltrg_list,ce->num_teltrg,iobuf); /* IDs of those telescopes */
      /* Trigger time (for first trigger if multiple trigger types fired) */
      put_vector_of_float(ce->teltrg_time,ce->num_teltrg,iobuf);
      put_short(ce->num_teldata,iobuf); /* Number of telescopes that produced data */
      put_vector_of_int(ce->teldata_list,ce->num_teldata,iobuf); /* IDs of those telescopes */
   }
   if ( item_header.version >= 2 )
   {
      /* Adding type-specific trigger times as each trigger type has its own inherent delay. */
      for ( itel=0; itel<ce->num_teltrg; itel++ )
      {
      ...
      {
         ntt = 0;
         for ( itrg=0; itrg<max_trg_types; itrg++ )
         {
            /* Check how many trigger types fired in this telescope */
         ...
         if ( ntt > 1 ) /* Need type-specific trigger times only with more than one type */
         {
            for ( itrg=0; itrg<max_trg_types; itrg++ )
            {
               /* Record type-specific times only for those types that actually triggered in this telescope */
               if ( (ce->teltrg_type_mask[itel] & (1<<itrg)) )
   ...
   if ( item_header.version >= 3 )
   {
      ...
      put_real(ce->az_comp,iobuf);   /* Azimuth angle */
      put_real(ce->alt_comp,iobuf);  /* Altitude angle */
      put_real(ce->ls_comp,iobuf);   /* Assumed speed of light in air */
   }

   return put_item_end(iobuf,&item_header);
}

/* -------------------- read_simtel_centralevent ------------------- */
/**
...
int read_simtel_centralevent (IO_BUFFER *iobuf, CentralEvent *ce)
{
   ...
   /* We can either assume that there are no more than four trigger types (version 2, 3)
      or we get told how many we need to support. */
   if ( item_header.version <= 3 && H_MAX_TRG_TYPES > 4 )
      max_trg_types = 4; /* Never more than 4 trigger types unless using version 4+ */
   if ( item_header.version >= 4 )
      max_trg_types = get_short(iobuf); /* How many different trigger types to be supported when reading the data */
   if ( max_trg_types > H_MAX_TRG_TYPES )
   {
      fprintf(stderr,"Request to read central event data block with up to %d trigger types but only %d supported.\n",
          max_trg_types, H_MAX_TRG_TYPES);
      max_trg_types = H_MAX_TRG_TYPES;
   }
   ...
   if ( item_header.version >= 2 )
   {
      ...
      for ( itel=0; itel< ce->num_teltrg; itel++ )
      {
         ...
         for ( itrg=0; itrg<max_trg_types; itrg++ )
         {
            /* Do we need to fix data produced by some problematic sim_telarray
               (versions between 2023-07-07 and 2024-02-25, not checked here ? */
            if ( (max_trg_types <= 4) && (ce->teltrg_type_mask[itel] & 0xf0) )
               ce->teltrg_type_mask[itel] =
                   (ce->teltrg_type_mask[itel] & ~0xf0) |
                   (ce->teltrg_type_mask[itel] & 0xf0)<<4; 
            if ( (ce->teltrg_type_mask[itel] & (1<<itrg)) )
            {
               ntt++;
   ...
   {
      /* Data relevant for undoing the plane wavefront compensation */
      ce->az_comp = get_real(iobuf);
      ce->alt_comp = get_real(iobuf);
      ce->ls_comp = get_real(iobuf);
   }
   else
   {
...
int print_simtel_centralevent (IO_BUFFER *iobuf)
{
   ...
   int teltrg_pattern =  get_int32(iobuf);
   printf("    Telescope trigger pattern: %d", teltrg_pattern);
   if ( teltrg_pattern != 0 )
   {
      printf(" (");
      int j=0;
   ...
   printf("\n");
   int teldata_pattern = get_int32(iobuf);
   printf("    Telescope data pattern:    %d", teldata_pattern);
   if ( teldata_pattern != 0 )
   {
      printf(" (");
      int j=0;
      for ( i=0; i<32; i++ )
      {
         if ( (teldata_pattern & (1 << i) ) != 0 )
         {
            if ( j )
               printf(", %d", i+1);
            else
   ...
   {
      max_trg_types = get_short(iobuf); /* How many different trigger types to be supported when reading the data */
      printf("    Number of trigger types to support: %d\n", max_trg_types);
   }
   if ( max_trg_types > H_MAX_TRG_TYPES )
   {
      fprintf(stderr,"Request to read central event data block with up to %d trigger types but only %d supported.\n",
   ...
   if ( item_header.version >= 2 )
   {
      ...
      for ( i=0; i<num_teltrg; i++ )
      {
         ...
         printf("    Telescope %d has trigger type mask: %d = 0x%x (", 
            teltrg_list[i], teltrg_type_mask[i], teltrg_type_mask[i]);
         ...
         /* Do we need to fix data produced by some problematic sim_telarray
            (versions between 2023-07-07 and 2024-02-25, not checked here ? */
         if ( (max_trg_types <= 4) && (teltrg_type_mask[i] & 0xf0) )
         {
            printf("%sbits 4-15 before bug fix: 0x%03x",lt?", ":"", (teltrg_type_mask[i]&0xfff0)>>4);
            lt = 1;
            teltrg_type_mask[i] =
                (teltrg_type_mask[i] & ~0xf0) |
                (teltrg_type_mask[i] & 0xf0)<<4;
            printf(" / fixed: 0x%03x", (teltrg_type_mask[i]&0xfff0)>>4);
         }
         if ( (teltrg_type_mask[i] & 0xff00) != 0 )
         {
            printf("%smodifiers: 0x%02x (",lt?", ":"", (teltrg_type_mask[i]&0xff00)>>8);
            lt = 0;
            if ( (teltrg_type_mask[i] & 0x0100) != 0 )
            {
               printf("%slong event",lt?", ":"");
               lt = 1;
            }
            if ( (teltrg_type_mask[i] & 0x0200) != 0 )
            {
               printf("%smuon mono",lt?", ":"");
               lt = 1;
            }
            if ( (teltrg_type_mask[i] & 0x0400) != 0 )
            {
               printf("%srandom mono",lt?", ":"");
               lt = 1;
            }
            if ( (teltrg_type_mask[i] & 0xf800) != 0 )
            {
               printf("%s...",lt?", ":"");
               lt = 1;
            }
   ...
   if ( item_header.version >= 3 )
   {
      double az_comp = get_real(iobuf);
      double alt_comp = get_real(iobuf);
      double ls_comp = get_real(iobuf);

      printf("    Plane wavefront compensation for az. = %5.3f deg, alt. = %5.3f deg, c/n = %f cm/ns.\n",
         az_comp*(180./M_PI), alt_comp*(180./M_PI), ls_comp);
   }

   return get_item_end(iobuf,&item_header);
}

/* -------------------- write_simtel_trackevent ------------------- */
/**
...
int write_simtel_trackevent (IO_BUFFER *iobuf, TrackEvent *tke)
{
   ...
   item_header.type = IO_TYPE_SIMTEL_TRACKEVENT + 
...
int read_simtel_trackevent (IO_BUFFER *iobuf, TrackEvent *tke)
{
   ...
   if ( (rc = get_item_begin(iobuf,&item_header)) < 0 )
      return rc;
   // tel_id = (item_header.type - IO_TYPE_SIMTEL_TRACKEVENT) % 100 +
   //         100 * ((item_header.type - IO_TYPE_SIMTEL_TRACKEVENT) / 1000);
...
int write_simtel_televt_head (IO_BUFFER *iobuf, TelEvent *te)
{
   ...
   item_header.type = IO_TYPE_SIMTEL_TELEVTHEAD;  /* Data type */
   item_header.version = 4;       /* Include event and trigger time, and position of start */
...
//      }
   ...
   if ( item_header.version >= 3 )
   {
      put_real(te->time_readout,iobuf);
      put_real(te->time_trg_rel,iobuf);
   }
   if ( item_header.version >= 4 )
   {
      put_int32(te->start_readout,iobuf);
   }
...
//      }
   ...
   if ( item_header.version == 4 )
   {
      /* Version number bumped to 4 when problem in sim_telarray conversion was fixed: */
      te->time_readout = get_real(iobuf);
      te->time_trg_rel = get_real(iobuf);
      te->start_readout = get_int32(iobuf);
   }
   else if ( item_header.version == 3 )
   {
      /* During the time when we recorded version 3, sim_telarray did
         the conversion incorrectly, affecting the following two numbers: */
      te->time_readout = get_real(iobuf);
      te->time_trg_rel = get_real(iobuf);
      /* Note that individual numbers may be wrong but the sum of them is correct. */
      te->start_readout= -1; /* Not known */
   }
   else
   {
      te->time_readout = 0.;
...
int print_simtel_televt_head (IO_BUFFER *iobuf)
{
   ...
   if ( (t & 0x100) )
   {
      ...
      if ( item_header.version >= 1 && (t & 0x400) )
      {
         ...
         printf(" ns");
         if ( trgmin < 99000. )
            printf("\n      Earliest trigger is %5.3f ns after start of simulation.", trgmin);
...
//      }
   ...
   if ( item_header.version == 3 )
   {
      /* During the time when we recorded version 3, sim_telarray did
         the conversion incorrectly, affecting the following two numbers: */
      double time_readout = get_real(iobuf);
      double time_trg_rel = get_real(iobuf);
      /* The individual numbers may be wrong but the sum is correct: */
      printf("      Trigger time: %5.3f ns (%5.3f + %5.3f ns).\n",
         time_readout+time_trg_rel, time_readout, time_trg_rel);
   }
   else if ( item_header.version >= 4 )
   {
      /* Version number bumped to 4 when problem was fixed: */
      double time_readout = get_real(iobuf);
      double time_trg_rel = get_real(iobuf);
      int start_readout = get_int32(iobuf);
      printf("      Readout starts at bin %d, global time %5.3f ns.\n",
         start_readout, time_readout);
      printf("      Trigger time: %5.3f ns after readout start, global time %5.3f ns.\n",
         time_trg_rel, time_readout+time_trg_rel);
   }
...
int write_simtel_teladc_sums (IO_BUFFER *iobuf, AdcData *raw)
{
   ...
   item_header.type = IO_TYPE_SIMTEL_TELADCSUM;  /* Data type */
   ...
   if ( item_header.version >= 2 )
   {
      put_long(raw->num_pixels,iobuf); /* Number of pixels always as (32-bit) 'long' = int32 */
...
int read_simtel_teladc_sums (IO_BUFFER *iobuf, AdcData *raw)
{
   ...
   if ( item_header.version == 0 )
   {
      /* Early version tried to squeeze multiple variables into one uint32 */
      raw->tel_id = (flags >> 25) & 0x1f; // High-order bits may be missing.
      raw->num_pixels = (flags >> 12) & 0x07ff; /* <= 2047 pixels */
   ...
   else if ( item_header.version == 1 )
   {
      /* Early version tried to squeeze multiple variables into one uint32 */
      raw->tel_id = (flags >> 25) & 0x1f; // High-order bits may be missing.
      raw->num_pixels = (flags >> 12) & 0x0fff; /* <= 4095 pixels */
      raw->num_gains = (((flags >> 24) & 0x01) ? 2 : 1);
   }
   else
   {
      /* Less squeezing, supporting more telescopes and more pixels */
...
int write_simtel_teladc_samples (IO_BUFFER *iobuf, AdcData *raw)
{
   ...
   item_header.type = IO_TYPE_SIMTEL_TELADCSAMP;  /* Data type */
   // item_header.version = 1;             /* Version 1 (revised) */
   ...
   if ( item_header.version >= 2 )
   {
      put_long(raw->num_pixels,iobuf); /* Number of pixels always as a 32-bit 'long' = int32 */
      put_short(raw->num_gains,iobuf);
   }

   put_short(raw->num_samples,iobuf);  /* Number of samples in one trace <= 32767 */
...
/* -------------------- write_simtel_aux_trace_digital ----------------- */
/**
 *  @short Write auxiliary digitized traces.
 *
...
int write_simtel_aux_trace_digital (IO_BUFFER *iobuf, AuxTraceD *auxd)
{
   ...
   if ( ! auxd->known || auxd->trace_data == NULL )
      return -1;
   if ( auxd->trace_type <= 0 || auxd->trace_type > MAX_AUX_TRACE_D )
   {
      fprintf(stderr, "Digital auxiliary trace type %d for telescope %d is out of range.\n",
         auxd->trace_type, auxd->tel_id);
      return -4;
   }

   item_header.type = IO_TYPE_SIMTEL_AUX_DIGITAL_TRACE;  /* Data type */
...
/* -------------------- read_simtel_aux_trace_digital ----------------- */
/**
 *  @short Read auxiliary digitized traces.
 *
*/  
...
int read_simtel_aux_trace_digital (IO_BUFFER *iobuf, AuxTraceD *auxd)
{
   ...
   if ( item_header.version > 0 )
   {
      fprintf(stderr,"Unsupported auxiliary digitized traces version: %d.\n",
         item_header.version);
      get_item_end(iobuf,&item_header);
      return -1;
   }
   ...
   if ( nt != auxd->num_traces || lt != auxd->len_traces || auxd->trace_data == NULL )
   {
      ...
      auxd->trace_data = (uint16_t *) malloc(nt*lt*sizeof(uint16_t));
      if ( auxd->trace_data == NULL )
      {
         fprintf(stderr,
            "Failed to allocate memory for %zu * %zu auxiliary digitized trace values.\n",
            nt, lt);
         get_item_end(iobuf,&item_header);
         return -1;
      }
...
/* -------------------- print_simtel_aux_trace_digital ----------------- */
/**
 *  @short Print auxiliary digitized traces.
 *
*/  
...
int print_simtel_aux_trace_digital (IO_BUFFER *iobuf)
{
   ...
   if ( item_header.version > 0 )
   {
      fprintf(stderr,"Unsupported auxiliary digitzed traces version: %d.\n",
         item_header.version);
      get_item_end(iobuf,&item_header);
      return -1;
   }
   ...
   printf("    Auxiliary digitized trace data (version %d) for telescope %d:\n", item_header.version, tel_id);
   ...
   if ( hs_verbose && len_traces <= H_MAX_SLICES )
   {
      ...
      for ( it=0; it<num_traces; it++ )
      {
         ...
         if ( it < (size_t)hs_maxprt )
         {
            printf("        DAux%d trace %zu:", trace_type, it);
...
/* -------------------- write_simtel_aux_trace_analog ----------------- */
/**
 *  @short Write auxiliary analog traces.
 *
*/  
...
int write_simtel_aux_trace_analog (IO_BUFFER *iobuf, AuxTraceA *auxa)
{
   ...
   if ( ! auxa->known || auxa->trace_data == NULL )
      return -1;
   if ( auxa->trace_type <= 0 || auxa->trace_type > MAX_AUX_TRACE_A )
   {
      fprintf(stderr, "Analog auxiliary trace type %d for telescope %d is out of range.\n",
         auxa->trace_type, auxa->tel_id);
      return -4;
   }

   item_header.type = IO_TYPE_SIMTEL_AUX_ANALOG_TRACE;  /* Data type */
   item_header.version = 0;
...
/* -------------------- read_simtel_aux_trace_analog ----------------- */
/**
 *  @short Read auxiliary analog traces.
 *
*/  
...
int read_simtel_aux_trace_analog (IO_BUFFER *iobuf, AuxTraceA *auxa)
{
   ...
   if ( item_header.version > 0 )
   {
      fprintf(stderr,"Unsupported auxiliary analog traces version: %d.\n",
         item_header.version);
      get_item_end(iobuf,&item_header);
      return -1;
   }
   ...
   if ( nt != auxa->num_traces || lt != auxa->len_traces || auxa->trace_data == NULL )
   {
      ...
      auxa->trace_data = (float *) malloc(nt*lt*sizeof(float));
      if ( auxa->trace_data == NULL )
      {
         fprintf(stderr,
            "Failed to allocate memory for %zu * %zu auxiliary analog trace values.\n",
            nt, lt);
         get_item_end(iobuf,&item_header);
         return -1;
      }
...
/* -------------------- print_simtel_aux_trace_analog ----------------- */
/**
 *  @short Print auxiliary analog traces.
 *
*/  
...
int print_simtel_aux_trace_analog (IO_BUFFER *iobuf)
{
   ...
   printf("    Auxiliary analog trace data (version %d) for telescope %d:\n", item_header.version, tel_id);
   printf("      Type %d with %ju traces of length %ju (time scale %f).\n",
      trace_type, num_traces, len_traces, time_scale);

   if ( hs_verbose )
   {
      size_t it, isamp, nshown = 0;
      float trace_data[len_traces];
      ...
      for ( it=0; it<num_traces; it++ )
      {
         ...
         for ( isamp=0; isamp<len_traces; isamp++ )
         {
            if ( trace_data[isamp] != 0. )
               empty = 0;
         }
         if ( empty )
            continue;
         // if ( it < (size_t)hs_maxprt )
         if ( nshown < (size_t)hs_maxprt )
         {
            /* Most traces can be expected to be empty and get skipped */
            printf("        AAux%d trace %zu:", trace_type, it);
            for ( isamp=0; isamp<len_traces; isamp++ )
...
int write_simtel_pixeltrg_time (IO_BUFFER *iobuf, PixelTrgTime *dt)
{
   ...
   item_header.type = IO_TYPE_SIMTEL_PIXELTRG_TM;  /* Data type */
   item_header.version = 0;
...
int print_simtel_pixeltrg_time (IO_BUFFER *iobuf)
{
   ...
   {
      size_t it;
      for ( it=0; it<(size_t)hs_maxprt && it<(size_t)num_times; it++ )
...
int write_simtel_pixtime (IO_BUFFER *iobuf, PixelTiming *pixtm)
{
   ...
   item_header.type = IO_TYPE_SIMTEL_PIXELTIMING;  /* Data type */
...
int read_simtel_pixtime (IO_BUFFER *iobuf, PixelTiming *pixtm)
{
   ...
   if ( item_header.version == 0 )
      v0 = 1;
   if ( item_header.version <= 1 )
      pixtm->num_pixels = get_short(iobuf); /* <= 32767 pixels with version 0 or 1 */
   else
      pixtm->num_pixels = get_scount32(iobuf); /* 32-bit number of pixels since version 2 */
...
int write_simtel_pixcalib (IO_BUFFER *iobuf, PixelCalibrated *pixcal)
{
   ...
   item_header.type = IO_TYPE_SIMTEL_PIXELCALIB;  /* Data type */
   ...
   put_count32((uint32_t) pixcal->num_pixels,iobuf); /* Actually an int32 stored as an uint32 of variable size */
...
int read_simtel_pixcalib (IO_BUFFER *iobuf, PixelCalibrated *pixcal)
{
   ...
   npix = (int) get_count(iobuf); /* 32-bit uint into an actual int(32) variable */
   if ( npix > H_MAX_PIX )
   {
...
int write_simtel_telimage (IO_BUFFER *iobuf, ImgData *img, int what)
{
   ...
   item_header.type = IO_TYPE_SIMTEL_TELIMAGE;  /* Data type */
   if ( img->pixels >= 32768)
...
int write_simtel_televent (IO_BUFFER *iobuf, TelEvent *te, int what)
{
   ...
   item_header.type = IO_TYPE_SIMTEL_TELEVENT + 
   ...
   if ( (rc = write_simtel_televt_head(iobuf,te)) < 0 )
   {
   ...
   if ( raw != NULL && raw->known && (what & (RAWDATA_FLAG|RAWSUM_FLAG)) != 0 )
   {
      if ( (te->readout_mode & 0xff) && (what & RAWDATA_FLAG) ) /* readout_mode is normally 0 (sum) or 1 (samples) or >=2 (both) */
      {
         ...
         if ( (te->readout_mode & 0xff) >= 2 ) /* Both sum and samples */
         {
            ...
            if ( (rc = write_simtel_teladc_sums(iobuf,raw)) == 0 )
               rc = write_simtel_teladc_samples(iobuf,raw);
      ...
      else /* Write only the sums */
      {
         ...
         rc = write_simtel_teladc_sums(iobuf,raw);
      }
      if ( rc < 0 )
      {
   ...
   {
      rc = write_simtel_pixtime(iobuf,te->pixtm);
      if ( rc < 0 )
      {
   ...
   {
      rc = write_simtel_pixcalib(iobuf,te->pixcal);
      if ( rc < 0 )
      {
   ...
   if ( img != NULL && img->known && (what & IMAGE_FLAG) != 0 )
   {
      ...
      {
         rc = write_simtel_telimage(iobuf,&img[j],what);
         if ( rc < 0 )
         {
   ...
   if ( te->trigger_pixels.pixels > 0 )
      write_simtel_pixel_list(iobuf,&te->trigger_pixels,te->tel_id);
   /* List of image pixels, if any */
   if ( te->image_pixels.pixels > 0 )
      write_simtel_pixel_list(iobuf,&te->image_pixels,te->tel_id);
   ...
   if ( te->pixeltrg_time.known && te->pixeltrg_time.num_times > 0 )
      write_simtel_pixeltrg_time(iobuf,&te->pixeltrg_time);
   ...
   for ( iaux=0; iaux < MAX_AUX_TRACE_D; iaux++ )
   {
      if ( te->aux_trace_d[iaux].known )
         write_simtel_aux_trace_digital(iobuf,&te->aux_trace_d[iaux]);
   }

   for ( iaux=0; iaux < MAX_AUX_TRACE_A; iaux++ )
   {
      if ( te->aux_trace_a[iaux].known )
         write_simtel_aux_trace_analog(iobuf,&te->aux_trace_a[iaux]);
   }

   return put_item_end(iobuf,&item_header);
}

/* ----------------------- read_simtel_televent ------------------------ */
/**
...
int read_simtel_televent (IO_BUFFER *iobuf, TelEvent *te, int what)
{
   ...
   if ( (rc = get_item_begin(iobuf,&item_header)) < 0 )
      return rc;
   tel_id = (item_header.type - IO_TYPE_SIMTEL_TELEVENT)%100 +
            100 * ((item_header.type - IO_TYPE_SIMTEL_TELEVENT)/1000);
   if ( tel_id < 0 || tel_id != te->tel_id )
   {
      Warning("Not a telescope event block or one for the wrong telescope");
      fprintf(stderr,"Found telescope ID %d, expected %d.\n",
         tel_id, te->tel_id);
      get_item_end(iobuf,&item_header);
      return -1;
   }
   ...
   if ( (rc = read_simtel_televt_head(iobuf,te)) < 0 )
   {
      get_item_end(iobuf,&item_header);
      return rc;
   }
   ...
   for (;;)
   {
      ...
      rc = -9; /* In case of any error not handle explicitly */

      switch ( nt )
      {
         case IO_TYPE_SIMTEL_TELADCSUM:
            ...
            rc = read_simtel_teladc_sums(iobuf,raw);
         ...
         case IO_TYPE_SIMTEL_TELADCSAMP:
            ...
            rc = read_simtel_teladc_samples(iobuf,raw,what);
            if ( rc == 0 )
            {
               raw->known |= 2;
            }
         ...
         case IO_TYPE_SIMTEL_PIXELTIMING:
            ...
            rc = read_simtel_pixtime(iobuf,te->pixtm);
            if ( rc != 0 )
               fprintf(stderr,"Error %d reading pixel timing sub-block for telescope %d.\n", rc, tel_id);
            break;

         case IO_TYPE_SIMTEL_PIXELCALIB:
            if ( te->pixcal == NULL )
            {
               if ( w_pixcal++ < 1 )
            ...
            rc = read_simtel_pixcalib(iobuf,te->pixcal);
            if ( rc != 0 )
               fprintf(stderr,"Error %d reading pixel calib sub-block for telescope %d.\n", rc, tel_id);
            break;

         case IO_TYPE_SIMTEL_TELIMAGE:
            ...
            if ( (rc = read_simtel_telimage(iobuf,&img[tel_img])) == 0 )
            {
               img[tel_img].known = 1;
               tel_img++;
            }
            else
               fprintf(stderr,"Error %d reading image %d sub-block for telescope %d.\n", rc, tel_img, tel_id);
            te->num_image_sets = tel_img;
            break;

         case IO_TYPE_SIMTEL_PIXELLIST:
         {
            ...
            {
               if ( (rc = read_simtel_pixel_list(iobuf,&te->trigger_pixels,&tid)) < 0 )
               {
                  fprintf(stderr,"Error %d reading trigger pixel list for telescope %d.\n", rc, tel_id);
                  get_item_end(iobuf,&item_header);
                  return rc;
               }
            }
            ...
            {
               if ( (rc = read_simtel_pixel_list(iobuf,&te->image_pixels,&tid)) < 0 )
               {
                  fprintf(stderr,"Error %d reading image pixel list for telescope %d.\n", rc, tel_id);
                  get_item_end(iobuf,&item_header);
                  return rc;
               }
            ...
            {
               int ixtrg = code - 10;
               if ( (rc = read_simtel_pixel_list(iobuf,&te->xtrigger_pixels[ixtrg],&tid)) < 0 )
               {
                  fprintf(stderr,"Error %d reading pixel list %d for telescope %d.\n", rc, code, tel_id);
                  get_item_end(iobuf,&item_header);
                  return rc;
               }
            }
         ...
         case IO_TYPE_SIMTEL_PIXELTRG_TM:
            te->pixeltrg_time.tel_id = te->tel_id;
            if ( ( rc = read_simtel_pixeltrg_time(iobuf,&te->pixeltrg_time) ) < 0 )
            {
               fprintf(stderr,"Error %d reading pixel trigger time for telescope %d.\n", rc, tel_id);
               get_item_end(iobuf,&item_header);
               return rc;
            }
            break;

         case IO_TYPE_SIMTEL_AUX_DIGITAL_TRACE:
            iaux = next_subitem_ident(iobuf);
            if ( iaux > 0 && iaux <= MAX_AUX_TRACE_D )
            {
               if ( (rc = read_simtel_aux_trace_digital(iobuf,&te->aux_trace_d[iaux-1])) < 0 )
               {
                  fprintf(stderr,"Error %d reading auxiliary digital trace %d for telescope %d\n", rc, iaux, tel_id);
                  get_item_end(iobuf,&item_header);
                  return rc;
               }
            }
            ...
            {
               fprintf(stderr,"Skipping auxiliary digital trace %d for telescope %d (out-of-range)\n", iaux, tel_id);
               rc = skip_subitem(iobuf);
            }
            break;

         case IO_TYPE_SIMTEL_AUX_ANALOG_TRACE:
            iaux = next_subitem_ident(iobuf);
            if ( iaux > 0 && iaux <= MAX_AUX_TRACE_A )
            {
               if ( (rc = read_simtel_aux_trace_analog(iobuf,&te->aux_trace_a[iaux-1])) < 0 )
               {
                 fprintf(stderr,"Error %d reading auxiliary analog trace %d for telescope %d\n", rc, iaux, tel_id);
                  get_item_end(iobuf,&item_header);
                  return rc;
               }
            }
            ...
            {
               fprintf(stderr,"Skipping auxiliary analog trace %d for telescope %d (out-of-range)\n", iaux, tel_id);
               rc = skip_subitem(iobuf);
            }
            break;
...
int print_simtel_televent (IO_BUFFER *iobuf)
{
   ...
   /* Telescope-specific event header is always used */
   if ( (rc = print_simtel_televt_head(iobuf)) < 0 )
   {
      get_item_end(iobuf,&item_header);
      return rc;
   }
   ...
   for (;;)
   {
      ...
      {
         case IO_TYPE_SIMTEL_TELADCSUM:
            rc = print_simtel_teladc_sums(iobuf);
            break;

         case IO_TYPE_SIMTEL_TELADCSAMP:
            rc = print_simtel_teladc_samples(iobuf);
            break;

         case IO_TYPE_SIMTEL_PIXELTIMING:
            rc = print_simtel_pixtime(iobuf);
            break;

         case IO_TYPE_SIMTEL_PIXELCALIB:
            rc = print_simtel_pixcalib(iobuf);
            break;

         case IO_TYPE_SIMTEL_TELIMAGE:
            rc = print_simtel_telimage(iobuf);
            break;

         case IO_TYPE_SIMTEL_PIXELLIST:
            rc = print_simtel_pixel_list(iobuf);
            break;

         case IO_TYPE_SIMTEL_PIXELTRG_TM:
            rc = print_simtel_pixeltrg_time(iobuf);
            break;

         case IO_TYPE_SIMTEL_AUX_DIGITAL_TRACE:
            rc = print_simtel_aux_trace_digital(iobuf);
            break;

         case IO_TYPE_SIMTEL_AUX_ANALOG_TRACE:
            rc = print_simtel_aux_trace_analog(iobuf);
            break;
...
int write_simtel_shower (IO_BUFFER *iobuf, ShowerParameters *sp)
{
   ...
   item_header.type = IO_TYPE_SIMTEL_SHOWER;  /* Data type */
...
int write_simtel_event (IO_BUFFER *iobuf, FullEvent *ev, int what)
{
   ...
   item_header.type = IO_TYPE_SIMTEL_EVENT;  /* Data type */
   item_header.version = 0;             /* Version 0 (test) */
   ...
   if ( ev->num_tel > 1 || ev->central.glob_count > 0 )
   ...
   rc = write_simtel_centralevent(iobuf,&ev->central);
   if ( rc != 0 )
   {
      Warning("Abort write_simtel_event() due to problem in write_simtel_centralevent()\n");
   ...
   if ( (what & (IMAGE_FLAG | RAWDATA_FLAG | RAWSUM_FLAG | TIME_FLAG)) )
   {
      for (j=0; j<ev->num_tel; j++)
      {
         if ( ev->teldata[j].known )
         {
            rc = write_simtel_televent(iobuf,&ev->teldata[j],what);
            if ( rc != 0 )
            {
               Warning("Abort write_simtel_event() due to problem in write_simtel_televent()\n");
               unput_item(iobuf,&item_header);
               return rc;
            }
         }
   ...
   if ( (what & (IMAGE_FLAG | RAWDATA_FLAG | RAWSUM_FLAG | TIME_FLAG | TRACKDATA_FLAG)) )
   {
      for (j=0; j<ev->num_tel; j++)
      {
         ...
         {
            rc = write_simtel_trackevent(iobuf,&ev->trackdata[j]);
            if ( rc != 0 )
            {
               Warning("Abort write_simtel_event() due to problem in write_simtel_trackevent()\n");
               unput_item(iobuf,&item_header);
               return rc;
            }
         }
   ...
   {
      rc = write_simtel_shower(iobuf,&ev->shower);
      if ( rc != 0 )
      {
         Warning("Abort write_simtel_event() due to problem in write_simtel_shower()\n");
         unput_item(iobuf,&item_header);
         return rc;
      }
   }
...
int read_simtel_event (IO_BUFFER *iobuf, FullEvent *ev, int what)
{
   ...
   {
      if ( type == IO_TYPE_SIMTEL_CENTEVENT )
      {
         if ( (rc = read_simtel_centralevent(iobuf,&ev->central)) < 0 )
         {
            get_item_end(iobuf,&item_header);
            Warning("Error reading central event data sub-block");
            return rc;
         }
      }
#if ( H_MAX_TEL > 100 )
      else if ( type >= IO_TYPE_SIMTEL_TRACKEVENT &&
         type%1000 < (IO_TYPE_SIMTEL_TRACKEVENT%1000)+100 &&
         (type-IO_TYPE_SIMTEL_TRACKEVENT)%100 +
         100*((type-IO_TYPE_SIMTEL_TRACKEVENT)/1000) <= H_MAX_TEL )
#else
      else if ( type >= IO_TYPE_SIMTEL_TRACKEVENT &&
         type <= IO_TYPE_SIMTEL_TRACKEVENT + H_MAX_TEL )
#endif
      {
         tel_id = (type - IO_TYPE_SIMTEL_TRACKEVENT)%100 +
                  100*((type-IO_TYPE_SIMTEL_TRACKEVENT)/1000);
         ...
         if ( (rc = read_simtel_trackevent(iobuf,&ev->trackdata[itel])) < 0 )
         {
            get_item_end(iobuf,&item_header);
            Warning("Error reading tracking data sub-block");
            return rc;
         }
      }
#if ( H_MAX_TEL > 100 )
      else if ( type >= IO_TYPE_SIMTEL_TELEVENT &&
         type%1000 < (IO_TYPE_SIMTEL_TELEVENT%1000)+100 &&
         (type-IO_TYPE_SIMTEL_TELEVENT)%100 +
         100*((type-IO_TYPE_SIMTEL_TELEVENT)/1000) <= H_MAX_TEL )
#else
      else if ( type >= IO_TYPE_SIMTEL_TELEVENT &&
         type <= IO_TYPE_SIMTEL_TELEVENT + H_MAX_TEL )
#endif
      {
         tel_id = (type - IO_TYPE_SIMTEL_TELEVENT)%100 +
                  100*((type-IO_TYPE_SIMTEL_TELEVENT)/1000);
         if ( (itel = find_tel_idx(tel_id)) < 0 )
         {
         ...
         if ( (rc = read_simtel_televent(iobuf,&ev->teldata[itel],what)) < 0 )
         {
            get_item_end(iobuf,&item_header);
            char line[1000];
            snprintf(line,sizeof(line)-1,"Error reading televent data sub-block for telescope ID %d.", tel_id);
            Warning(line);
            return rc;
         }
      ...
      else if ( type == IO_TYPE_SIMTEL_SHOWER )
      {
         if ( (rc = read_simtel_shower(iobuf,&ev->shower)) < 0 )
         {
            get_item_end(iobuf,&item_header);
            Warning("Error reading shower data sub-block");
            return rc;
         }
      }
      else
...
int print_simtel_event (IO_BUFFER *iobuf)
{
   ...
   while ( (type = next_subitem_type(iobuf)) > 0 )
   {
      if ( type == IO_TYPE_SIMTEL_CENTEVENT )
      {
         if ( (rc = print_simtel_centralevent(iobuf)) < 0 )
         {
            get_item_end(iobuf,&item_header);
            return rc;
         }
...
#endif
      {
         ...
         if ( (rc = print_simtel_trackevent(iobuf)) < 0 )
         {
            get_item_end(iobuf,&item_header);
            return rc;
         }
...
#if 1
         if ( (rc = print_simtel_televent(iobuf)) < 0 )
         {
            get_item_end(iobuf,&item_header);
            return rc;
         }
      ...
      {
#if 1
         if ( (rc = print_simtel_shower(iobuf)) < 0 )
         {
            get_item_end(iobuf,&item_header);
            return rc;
         }
...
int write_simtel_calib_event (IO_BUFFER *iobuf, FullEvent *ev, int what, int type)
{
   ...
   item_header.type = IO_TYPE_SIMTEL_CALIBEVENT;  /* Data type */
   item_header.version = 0;             /* Version 0 (test) */
   item_header.ident = type;

#ifdef NO_CALIBRATION_WRAPPING
   return write_simtel_event(iobuf,ev,what);
#else
   put_item_begin(iobuf,&item_header);
   
   write_simtel_event(iobuf,ev,what);

   return put_item_end(iobuf,&item_header);
#endif
}

/* ------------------- read_simtel_calib_event -------------------- */
/**
...
int read_simtel_calib_event (IO_BUFFER *iobuf, FullEvent *ev, int what, int *ptype)
{
   ...
   read_simtel_event(iobuf,ev,what);   

   return get_item_end(iobuf,&item_header);
}

/* ------------------- print_simtel_calib_event -------------------- */
/**
...
int print_simtel_calib_event (IO_BUFFER *iobuf)
{
   ...
   print_simtel_event(iobuf); /* print contained event */   

   return get_item_end(iobuf,&item_header);
}

/* ------------------- read_simtel_calib_pe -------------------- */
/**
 *  @short Read photo-electrons for a calibration event (pedestal, laser, led, ...) as
 *         an encapsulated raw data event.
 */
...
int read_simtel_calib_pe (IO_BUFFER *iobuf, MCEvent *mce, int *ptype)
{
   ...
   item_header.type = IO_TYPE_SIMTEL_CALIB_PE;  /* Data type */
   if ( (rc = get_item_begin(iobuf,&item_header)) < 0 )
      return rc;
   if ( item_header.version != 0 )
   {
      fprintf(stderr,"Unsupported calibration p.e. list data version: %d.\n",
         item_header.version);
      get_item_end(iobuf,&item_header);
      return -1;
   }
   ...
   read_simtel_mc_phot(iobuf,mce);   

   return get_item_end(iobuf,&item_header);
}

/* ------------------- print_simtel_calib_pe -------------------- */
/**
 *  @short Print a p.e. list for an internal calibration event from an encapsulated data block.
 */
...
int print_simtel_calib_pe (IO_BUFFER *iobuf)
{
   ...
   if ( item_header.version != 0 )
   {
      fprintf(stderr,"Unsupported calibration p.e. data version: %d.\n",
         item_header.version);
      get_item_end(iobuf,&item_header);
      return -1;
   }
   type = item_header.ident;
   printf("\nCalibration p.e. list for type %d:\n", type);

   print_simtel_mc_phot(iobuf); /* print contained event */   

   return get_item_end(iobuf,&item_header);
}

/* --------------------- write_simtel_mc_shower -------------------- */
/**
...
int write_simtel_mc_shower (IO_BUFFER *iobuf, MCShower *mcs)
{
   ...
   item_header.type = IO_TYPE_SIMTEL_MC_SHOWER;  /* Data type */
...
int write_simtel_mc_event (IO_BUFFER *iobuf, MCEvent *mce)
{
   ...
   item_header.type = IO_TYPE_SIMTEL_MC_EVENT;  /* Data type */
   if ( mce->aweight == 0. )
...
int write_simtel_mc_pe_sum (IO_BUFFER *iobuf, MCpeSum *mcpes)
{
   ...
   item_header.type = IO_TYPE_SIMTEL_MC_PE_SUM;  /* Data type */
...
/* ------------------ write_simtel_mc_pixel_moni ---------- */

int write_simtel_mc_pixel_moni (IO_BUFFER *iobuf, MCPixelMonitor *mcpixmon)
{
   IO_ITEM_HEADER item_header;

   if ( iobuf == (IO_BUFFER *) NULL || mcpixmon == NULL )
      return -1;

   if ( mcpixmon->flags == 0 || mcpixmon->num_pixels <= 0 || 
        mcpixmon->num_gains <= 0 || mcpixmon->flags > 0x1ff ||
        mcpixmon->num_pixels > H_MAX_PIX || mcpixmon->num_gains > H_MAX_GAINS )
   {
      fflush(stdout);
      Warning("MC Pixel Monitor block not written because of invalid parameters.");
      fprintf(stderr,"  tel_id = %d\n", mcpixmon->tel_id);
      fprintf(stderr,"  flags  = %d\n", mcpixmon->flags);
      fprintf(stderr,"  num_pixels = %d (max = %d)\n", mcpixmon->num_pixels, H_MAX_PIX);
      fprintf(stderr,"  num_gains = %d (max = %d)\n\n", mcpixmon->num_gains, H_MAX_GAINS);
      return -1;
   }

   item_header.type = IO_TYPE_SIMTEL_MC_PIXMON;  /* Data type */
   item_header.version = 0;             /* Version number */
   item_header.ident = mcpixmon->tel_id;
   put_item_begin(iobuf,&item_header);

   put_int32(mcpixmon->flags & 0x1ff, iobuf);
   put_int32(mcpixmon->num_pixels, iobuf); /* Number of pixels always as int32 */
   put_short(mcpixmon->num_gains, iobuf);

   if ( (mcpixmon->flags & 0x01) != 0 )
      put_vector_of_real(mcpixmon->nsb_pe_rate,mcpixmon->num_pixels,iobuf);
   if ( (mcpixmon->flags & 0x02) != 0 )
      put_vector_of_real(mcpixmon->qe_rel,mcpixmon->num_pixels,iobuf);
   if ( (mcpixmon->flags & 0x04) != 0 )
      put_vector_of_real(mcpixmon->gain_rel,mcpixmon->num_pixels,iobuf);
   if ( (mcpixmon->flags & 0x08) != 0 )
      put_vector_of_real(mcpixmon->hv_rel,mcpixmon->num_pixels,iobuf);
   if ( (mcpixmon->flags & 0x10) != 0 )
      put_vector_of_real(mcpixmon->current,mcpixmon->num_pixels,iobuf);
   if ( (mcpixmon->flags & 0x20) != 0 )
      put_vector_of_real(mcpixmon->fadc_amp[0],mcpixmon->num_pixels,iobuf);
#if H_MAX_GAINS > 1
   if ( (mcpixmon->flags & 0x40) != 0 && mcpixmon->num_gains > 1 )
      put_vector_of_real(mcpixmon->fadc_amp[1],mcpixmon->num_pixels,iobuf);
#endif
   if ( (mcpixmon->flags & 0x80) != 0 )
      put_vector_of_byte(mcpixmon->disabled,mcpixmon->num_pixels,iobuf);
   if ( (mcpixmon->flags & 0x100) != 0 )
      put_vector_of_real(mcpixmon->delay,mcpixmon->num_pixels,iobuf);

   return put_item_end(iobuf,&item_header);
}

/* ------------------ read_simtel_mc_pixel_moni ---------- */
...
int read_simtel_mc_pixel_moni (IO_BUFFER *iobuf, MCPixelMonitor *mcpixmon)
{
   ...
   {
      Warning("MC Pixel Monitor block is for wrong telescope");
      get_item_end(iobuf,&item_header);
      return -1;
   }

   mcpixmon->tel_id = item_header.ident;
   flags = get_int32(iobuf);
   npix = get_int32(iobuf);
   ngain = get_short(iobuf);
   ...
   {
      Warning("MC Pixel Monitor block has invalid number of pixels");
      get_item_end(iobuf,&item_header);
      return -1;
   }
   ...
   {
      Warning("MC Pixel Monitor block has invalid number of gains");
      get_item_end(iobuf,&item_header);
      return -1;
   }
   ...
   if ( (mcpixmon->flags & 0x01) != 0 )
      get_vector_of_real(mcpixmon->nsb_pe_rate,mcpixmon->num_pixels,iobuf);
   if ( (mcpixmon->flags & 0x02) != 0 )
      get_vector_of_real(mcpixmon->qe_rel,mcpixmon->num_pixels,iobuf);
   if ( (mcpixmon->flags & 0x04) != 0 )
      get_vector_of_real(mcpixmon->gain_rel,mcpixmon->num_pixels,iobuf);
   if ( (mcpixmon->flags & 0x08) != 0 )
      get_vector_of_real(mcpixmon->hv_rel,mcpixmon->num_pixels,iobuf);
   if ( (mcpixmon->flags & 0x10) != 0 )
      get_vector_of_real(mcpixmon->current,mcpixmon->num_pixels,iobuf);
   if ( (mcpixmon->flags & 0x20) != 0 )
      get_vector_of_real(mcpixmon->fadc_amp[0],mcpixmon->num_pixels,iobuf);
#if H_MAX_GAINS > 1
   if ( (mcpixmon->flags & 0x40) != 0 && ngain > 1 )
      get_vector_of_real(mcpixmon->fadc_amp[1],mcpixmon->num_pixels,iobuf);
#endif
   if ( (mcpixmon->flags & 0x80) != 0 )
      get_vector_of_byte(mcpixmon->disabled,mcpixmon->num_pixels,iobuf);
   if ( (mcpixmon->flags & 0x100) != 0 )
      get_vector_of_real(mcpixmon->delay,mcpixmon->num_pixels,iobuf);

   return get_item_end(iobuf,&item_header);
}

/* ------------------ print_simtel_mc_pixel_moni ---------- */

int print_simtel_mc_pixel_moni (IO_BUFFER *iobuf)
{
   IO_ITEM_HEADER item_header;
   int rc, i;
   int tel_id, flags, npix, ngain;
   ...
   printf("\nMC Pixel Monitor block for telescope ID %d with %d pixels of %d gains, content flags = %d\n",
      tel_id, npix, ngain, flags);

   if ( (flags & 0x01) != 0 )
   {
      printf("   NSB pixel p.e. rates =");
      for (i=0; i<npix; i++)
      {
         if ( i < hs_maxprt )
            printf(" %f,", get_real(iobuf));
...
int write_simtel_tel_monitor (IO_BUFFER *iobuf, TelMoniData *mon, int what)
{
   ...
   item_header.type = IO_TYPE_SIMTEL_TEL_MONI;  /* Data type */
   ...
   if ( (what & 0x04) ) /* Pedestals + noise */
   {
      ...
      {
         /* Were pedestals given per sample but not per sum? */
         if ( mon->pedestal[j][0] == 0. && mon->pedsamp[j][0] != 0. )
         {
            int i;
            for (i=0; i<mon->num_pixels; i++)
               mon->pedestal[j][i] = mon->pedsamp[j][i] * (double) mon->num_ped_slices;
         }
         /* Recorded is always the pedestal sum */
   ...
   {
      int j;
      put_time_blob(&mon->set_pedcomp_time,iobuf);
      for ( j=0; j<mon->num_gains; j++ )
      {
         /* Raw pedestals = reported pedestals minus these compensation values */
         put_vector_of_int32(mon->ped_comp_rel[j],mon->num_pixels,iobuf);
      }
   }
...
int read_simtel_tel_monitor (IO_BUFFER *iobuf, TelMoniData *mon)
{
   ...
   if ( (what & 0x04) ) /* Pedestals + noise */
   {
      ...
      for ( j=0; j<mon->num_gains; j++ )
      {
         ...
         {
            for ( i=0; i<mon->num_pixels; i++ )
               mon->pedsamp[j][i] = mon->pedestal[j][i] / (double) mon->num_ped_slices;
         }
         else
         {
            for ( i=0; i<mon->num_pixels; i++ )
   ...
   {
      int j;
      get_time_blob(&mon->set_pedcomp_time,iobuf);
      for ( j=0; j<mon->num_gains; j++ )
      {
         get_vector_of_int32(mon->ped_comp_rel[j],mon->num_pixels,iobuf);
      }
   }
...
int print_simtel_tel_monitor (IO_BUFFER *iobuf)
{
   ...
   if ( item_header.version == 0 )
   {
      ns = get_short(iobuf);  /* mon->num_sectors */
      np = get_short(iobuf);  /* mon->num_pixels */
      nd = get_short(iobuf);  /* mon->num_drawers */
      ng = get_short(iobuf);  /* mon->num_gains */
   }
   else
   {
      ns = get_scount32(iobuf);  /* mon->num_sectors */
      np = get_scount32(iobuf);  /* mon->num_pixels */
      nd = get_scount32(iobuf);  /* mon->num_drawers */
      ng = get_scount32(iobuf);  /* mon->num_gains */
   }
   ...
   if ( (what & 0x04) ) /* Pedestals + noise */
   {
      ...
      for ( j=0; j<ng; j++ )
      {
         printf("      Pedestals gain %d: sum (per sample): ",j);
         for (i=0; i<np; i++)
         {
            double ped = get_real(iobuf);
            if ( i < hs_maxprt || i == np-1 )
               printf(" %f (%f),", ped, ped/num_ped_slices);
            else if ( i == hs_maxprt )
               printf(" ...");  
         }
         printf("\n");
   ...
   {
      get_time_blob(&mtime,iobuf);
      printf("   HV + temperatures (CNTRLMon): time: %ld.%09ld:\n", 
         mtime.seconds, mtime.nanoseconds);
      printf("        ...\n");
      int ndtemp = get_short(iobuf); /* mon->num_drawer_temp */
      int nctemp = get_short(iobuf); /* mon->num_camera_temp */
      int j;
      for ( j=0; j<np; j++ )
         (void) get_uint16(iobuf); /* mon->hv_v_mon */
      for ( j=0; j<np; j++ )
         (void) get_uint16(iobuf); /* mon->hv_i_mon */
      for ( j=0; j<np; j++ )
         (void) get_byte(iobuf); /* mon->hv_stat */
      for ( j=0; j<nd; j++ )
      {
         for (i=0; i<ndtemp; i++)
            (void) get_short(iobuf); /* mon->drawer_temp[j][i] */
      }
      for (i=0; i<nctemp; i++)
         (void) get_short(iobuf); /* mon->camera_temp[i] */
   }
   if ( (what & 0x10) ) /* Pixel scalers + DC currents (CNTRLCpt) */
   {
      get_time_blob(&mtime,iobuf);
      printf("   Pixel scaler + DC currents (CNTRLCpt): time: %ld.%09ld:\n", 
         mtime.seconds, mtime.nanoseconds);
      printf("        ...\n");
      int j;
      for ( j=0; j<np; j++ )
         (void) get_uint16(iobuf); /* mon->current */
      for ( j=0; j<np; j++ )
         (void) get_uint16(iobuf); /* mon->scaler */
   }
   if ( (what & 0x20) ) /* HV + thresholds settings (CNTRLSc+CNTRLHt) */
   {
      get_time_blob(&mtime,iobuf);
      ...
      for ( i=0; i<np; i++ )
      {
         unsigned int v = get_uint16(iobuf);
         if ( i < hs_maxprt || i == np-1 )
            printf(" %u,",v);
         else if ( i == hs_maxprt )
            printf(" ...");
      ...
      for ( i=0; i<np; i++ )
      {
         unsigned char c = get_byte(iobuf);
         if ( i < hs_maxprt || i == np-1 )
            printf(" %u,", c);
         else if ( i == hs_maxprt )
            printf(" ...");
   ...
   if ( (what & 0x80) ) /* Pedestal compensation */
   {
      ...
      int j;
      get_time_blob(&mtime,iobuf);
      printf("   Pedestal compensation: time: %ld.%09ld:\n", 
         mtime.seconds, mtime.nanoseconds);
      for ( j=0; j<ng; j++ )
      {
         printf("      Pedestals for gain %d compensated by:", j);
         for (i=0; i<np; i++)
         {
            int pcrg = get_int32(iobuf);
            if ( i < hs_maxprt || i == np-1 )
               printf(" %d,", pcrg);
            else if ( i == hs_maxprt )
               printf(" ...");
...
//      item_header.version = 1;          /* need version 1 */
   ...
   if ( item_header.version >= 3 )
      for ( j=0; j<lcd->num_gains; j++ )
         put_vector_of_real(lcd->ff_corr[j],lcd->num_pixels,iobuf);

   return put_item_end(iobuf,&item_header);
}


/* --------------------- read_simtel_laser_calib ---------------------- */
/**
...
int read_simtel_laser_calib (IO_BUFFER *iobuf, LasCalData *lcd)
{
   ...
   else
   {
      ...
      if ( np < 0 )
      {
         /* Some data with 32768 <= np <= 65535 pixels got produced before proper
            support for them was available with version 4 format.
            Trying a work-around by interpreting the data as unsigned short (uint16_t). */
         char message[1024];
         uint16_t nu = (uint16_t) np;
         sprintf(message,"Laser calibration for telescope %d: Number of pixels out of range. Trying unsigned short (%u) instead of int (%d).",
            lcd->tel_id, nu, np);
         fflush(stdout);
         Warning(message);
         np = (int) nu;
   ...
   {
      fflush(stdout);
      Warning("Laser calibration data is for different setup. Trying to proceed anyway.");
   }
   lcd->num_pixels = np;
   lcd->num_gains = ng;
   ...
   {
      fflush(stdout);
      Warning("Laser calibration data is for bad setup (pixels or gains out of range).");
      get_item_end(iobuf,&item_header);
      return -1;
   }
   ...
   {
      for ( j=0; j<lcd->num_gains; j++ )
         get_vector_of_real(lcd->ff_corr[j],lcd->num_pixels,iobuf);
   }
...
int print_simtel_laser_calib (IO_BUFFER *iobuf)
{
   ...
   else
   {
      ...
      if ( np < 0 )
      {
         ...
         char message[1024];
         uint16_t nu = (uint16_t) np;
         sprintf(message,"Laser calibration for telescope %ld: Number of pixels out of range. Trying unsigned short (%u) instead of int (%d).",
            item_header.ident, (unsigned) nu, np);
         fflush(stdout);
         Warning(message);
         np = (int) nu;
      }
   ...
   if ( item_header.version >= 1 )
   {
      for ( j=0; j<ng; j++ )
      {
         double max_int_frac = get_real(iobuf);
         double max_pixtm_frac = get_real(iobuf);
         if ( max_int_frac > 0. && max_pixtm_frac > 0. )
         {
            printf("   Readout/integration can see fractions <= %f (%f) of the total signal "
                   "within fixed window (in peak finding window) at gain %d\n",
               max_int_frac, max_pixtm_frac, j);
         }
         else if ( max_int_frac > 0. )
         {
            printf("   Readout/integration can see a fraction <= %f of the total signal "
                   "within fixed window at gain %d\n",
               max_int_frac, j);
         }
         else
         {
            printf("   Fraction of gain %d pulse shape seen in readout/integration window is not available.\n", j);
         }
      }
   }
...
int write_simtel_run_stat (IO_BUFFER *iobuf, RunStat *rs)
{
   ...
   item_header.type = IO_TYPE_SIMTEL_RUNSTAT;  /* Data type */
   item_header.version = 0;             /* Version 0 (test) */
...
int write_simtel_mc_run_stat (IO_BUFFER *iobuf, MCRunStat *mcrs)
{
   ...
   item_header.type = IO_TYPE_SIMTEL_MC_RUNSTAT;  /* Data type */
   item_header.version = 0;             /* Version 0 (test) */
...
int print_simtel_mc_phot (IO_BUFFER *iobuf)
{
   ...
   if ( (rc = begin_read_tel_array(iobuf, &item_header, &iarray)) < 0 )
      return rc;
   printf("\nMC photon or photo-electron data for array/event %d\n", iarray);
   while ( (type = next_subitem_type(iobuf)) > 0 )
   {
      switch (type)
      {
...
int write_simtel_pixel_list (IO_BUFFER *iobuf, PixelList *pl, int telescope)
{
   ...
   item_header.type = IO_TYPE_SIMTEL_PIXELLIST;  /* Data type */
   if ( pl->pixels >= 32768 )
