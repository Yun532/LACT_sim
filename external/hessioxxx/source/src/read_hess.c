...
Copyright (C) 2001, 2002, 2003, 2004, 2005, 2006-2021, 2022, 2023, 2024  Konrad Bernloehr
 ...
 *  format (which is what the read_simtel_nr program is now intended for).
...
Options:
   ...
   --single        (Same as '--min-tel 1 --min-trg-tel 1'.)
...
/** @defgroup read_simtel_c The read_hess (aka read_simtel, read_cta) program */
/** @{ */
...
static void mc_event_fill (AllHessData *hsdata, double d_sp_idx)
{
   ...
   if ( hsdata->mc_event.aweight != 0. )
      ewt *= hsdata->mc_event.aweight;
...
static void syntax (char *program)
{
   ...
   printf("   --plot-without-pix-cross (No crosses marking trigger and image pixels.)\n");
   ...
   printf("   --only-event ev1[,ev2-ev3[,...]] (Select specific events.)\n");
   printf("   --only-shower n1[,n2-n3[,...]] (Select specific showers.)\n");
   printf("   --only-run run1[,run2-run3[,...]] (Select runs being processed.)\n");
   printf("   --not-run run1[,run2-run3[,...]]\n");
   printf("   --only-type t1[,t2[,...]] (Select telescopes of given type(s) only.)\n");
   printf("   --only-telescope id1[,id2-i3[,...]] (Select telescopes being used.)\n");
   printf("   --not-telescope id1[,id2-id3[,...]]\n");
   printf("   --required-telescope id (A specific telescope which has to have data.)\n");
   ...
   printf("   --max-trg-tel n (Maximum number of telescopes in system trigger.)\n");
   printf("   --single        (Same as '--min-tel 1 --min-trg-tel 1'.)\n");
   printf("   --stereo        (Same as '--min-tel 2 --min-trg-tel 2'. Default setting.)\n");
   ...
   printf("   --broken-pixels-list (Replace broken pixels with pre-generated lists.)\n");
   ...
   printf("   --pixel-stats   *(Fill histograms of pixel trigger statistics.)\n");
...
int read_disabled_pixels_list (const char *fname, PixelDisabled **list)
{
   // printf("\nread_disabled_pixels()\n");
   if ( fname == NULL || list == NULL )
   {
      printf("Invalid parameters to read_disabled_pixels_list(). Not getting any disabled pixels.\n");
      return -1;
   }
   ...
   {
      perror(fname);
      printf("Not getting any disabled pixels.\n");
      return -1;
   }

   printf("\nManual list of disabled pixels to be read from file %s\n", fname);

   IO_BUFFER *iobuf = allocate_io_buffer(100000L); /* Use local buffer not the main buffer */
   if ( iobuf == NULL )
   {
      fileclose(f);
      printf("Allocation of I/O buffer failed. Not getting any disabled pixels.\n");
      return -2;
   }
   iobuf->input_file = f;
   IO_ITEM_HEADER item_header;
   item_header.type = 0; /* Look for any type */
   if ( find_io_block(iobuf,&item_header) != 0 )
   {
      /* File may be empty or contain something else but I/O blocks. */
      fileclose(f);

      printf("File %s does not contain EventIO data blocks, trying text mode ...\n", fname);
      /* We might re-open it and try it as a text list ... */
      f = fileopen(fname,"r");
      if ( f == NULL )
      {
         printf("Re-opening file %s for scanning a text-mode list of disabled pixels failed.\n", fname);
         return -1;
      }
      char line[10000], word[100];
      while ( fgets(line,sizeof(line)-1,f) != NULL )
      {
         int ipos = 0;
         if ( getword(line,&ipos,word,sizeof(word)-1,':','\n') <= 0 )
            continue;
         int tel_id = atoi(word);
         int itel = -1;
         if ( (itel = find_tel_idx(tel_id)) < 0 )
         {
            char msg[256];
            snprintf(msg,sizeof(msg)-1,
               "Pixel disable list for unknown telescope ID %d.", tel_id);
            Warning(msg);
            continue;
         }
         // printf("Found disabled pixel(s) for telescope %d (itel=%d).\n", tel_id, itel);
         
         if ( list[itel] == NULL ) /* Allocation on demand only */
         {
            if ( (list[itel] = malloc(sizeof(PixelDisabled))) == NULL )
            {
               Warning("Allocation error for disabled pixels list.");
               exit(1);
            }
            list[itel]->tel_id = tel_id;
            list[itel]->num_trig_disabled = 0;
            list[itel]->num_HV_disabled = 0;
         }

         while ( getword(line,&ipos,word,sizeof(word)-1,',','\n') > 0 )
         {
            int p1 = -1, p2 = -1, n = 0, ip = 0;
            if ( strchr(word,'-') != NULL )
            {
               n = sscanf(word,"%d-%d",&p1,&p2);
               if ( n == 2 && p1 >= 0 && p2 >= 0 && p2 >= p1 && p2 < H_MAX_PIX )
               {
                  // printf("   Disable pixels %d to %d\n", p1, p2);
                  for ( ip=p1; ip<=p2; ip++ )
                  {
                     if ( list[itel]->num_HV_disabled < H_MAX_PIX )
                        list[itel]->HV_disabled[list[itel]->num_HV_disabled++] = ip;
                     else
                        break; /* Only possible if there are too many pixel duplicates */
                  }
               }
            }
            else
            {
               n = sscanf(word,"%d",&p1);
               if ( n == 1 && p1 >= 0 && p1 < H_MAX_PIX )
               {
                     // printf("   Disable pixel %d\n", p1);
                     if ( list[itel]->num_HV_disabled < H_MAX_PIX )
                        list[itel]->HV_disabled[list[itel]->num_HV_disabled++] = p1;
               }
            }
         }
      }
   ...
   do
   {
      if ( item_header.type != IO_TYPE_SIMTEL_PIXELDISABLE )
      {
      ...
      int tel_id = item_header.ident; // Telescope ID is in the header
      int itel = -1;
      if ( (itel = find_tel_idx(tel_id)) < 0 )
      {
         char msg[256];
      ...
      int rc = read_simtel_pixeldis(iobuf,list[itel]);
      if ( rc != 0 )
         printf("read_simtel_pixeldis(), rc = %d (tel. ID=%d, itel=%d)\n",rc,tel_id,itel);
...
/* ---------------- show_header --------------- */
/**
  * @short Print (to stdout) what information we have in the item header.
  */
...
int main (int argc, char **argv)
{
   ...
   int req_tel = -1;
   int nev = 0, ntrg = 0, nrun = 0, nev_tot = 0, ntrg_tot = 0;
   char *program = argv[0];
   char base_program[1024];
   int showdata = 0, showhistory = 0;
   int clean_history = 0;
   int flag_amp_tm = 0;
   size_t num_only = 0, num_not = 0, num_onlytype = 0;
   ...
   double hmax_cut = 1.;
   size_t min_pix = 2, num_tel_img = 0, min_tel_img = 2, max_tel_img = 999;
   size_t num_tel_raw = 0;
   int user_ana = 0;
   ...
   double calerror = 0.;
   static PixelDisabled *manual_pixel_disabled[H_MAX_TEL]; 
   const char *disabled_list_fname = NULL;
   int disabled_list_done = 0;
   ...
   static RangeList only_events = { 0, 0, NULL };
   static RangeList only_showers = { 0, 0, NULL };
   int skip_run = 0, skip_shower = 0;
   ...
   if ( reco_verbose_level > 1 )
      setenv("PRINT_TEL_VERBOSE","1",1);
   if ( ! quiet )
      show_hessio_max();
   ...
   if ( iobuf->max_length < 200000000L )
   ...
   while ( argc > 1 )
   {
      ...
      else if ( strcmp(argv[1],"--plot-without-pix-cross") == 0 )
      {
         setenv("PLOT_WITHOUT_PIX_CROSS","1",1);
         argc--;
         argv++;
         continue;
      }
...
#undef SHOW
      ...
      else if ( (strcmp(argv[1],"--dst-file") == 0 || strcmp(argv[1],"--raw-file") == 0 ||
         strcmp(argv[1],"--output-file") == 0 || strcmp(argv[1],"-o") == 0) && argc > 2 )
      {
         dst_fname = argv[2];
         argc -= 2;
         argv += 2;
      ...
      else if ( strcmp(argv[1],"--broken-pixels-list") == 0 && argc > 2 )
      {
         disabled_list_fname = argv[2];
         disabled_list_done = 0;
         argc -= 2;
         argv += 2;
         continue;
      }
      ...
      else if ( ( strcmp(argv[1],"--only-type") == 0 ||
                  strcmp(argv[1],"--only-types") == 0 ) &&
                argc > 2 )
      {
         char word[20];
         int ipos=0;
                 ...
                 num_onlytype < 10 )
         {
            ...
            if ( ntp < 1 || ntp > 10 )
            {
               fprintf(stderr,"Requested telescope type number %d is out of range.\n", ntp);
               ntp = -1;
               continue;
            }
            for ( ii=0; ii<num_onlytype; ii++ )
            {
               if ( only_type[ii] == ntp )
               {
                  ntp = -1;
      ...
      else if ( ( strcmp(argv[1],"--required-telescope") == 0 ||
                  strcmp(argv[1],"--req-telescope") == 0 ) &&
                argc > 2 )
      {
         req_tel = atoi(argv[2]);
         argc -= 2;
      ...
      else if ( ( strcmp(argv[1],"--only-event") == 0 ||
                  strcmp(argv[1],"--only-events") == 0 ) &&
                argc > 2 )
      {
         char word[20];
         int ipos=0;
         while ( getword(argv[2],&ipos,word,sizeof(word)-1,',','\n') > 0 )
         {
            long evt_idx = atol(word), evt_idx2;
            char *sl;
            if ( evt_idx > 0 )
            {
               evt_idx2 = evt_idx;
               ...
               add_range(evt_idx, evt_idx2, &only_events);
            }
         }
         argc -= 2;
         argv += 2;
      ...
      else if ( ( strcmp(argv[1],"--only-shower") == 0 ||
                  strcmp(argv[1],"--only-showers") == 0 ) &&
                argc > 2 )
      {
         char word[20];
         int ipos=0;
         while ( getword(argv[2],&ipos,word,sizeof(word)-1,',','\n') > 0 )
         {
            long shw_idx = atol(word), shw_idx2;
            char *sl;
            if ( shw_idx > 0 )
            {
               shw_idx2 = shw_idx;
               ...
               add_range(shw_idx, shw_idx2, &only_showers);
            }
         }
         argc -= 2;
         argv += 2;
      ...
      else if ( strcmp(argv[1],"--pixel-stats") == 0 && argc > 2 )
      {
         int on = 0;
         if ( atoi(argv[2]) != 0 )
            on = 1;
         user_set_pixel_stats(on);
         argc -= 2;
         argv += 2;
         continue;
      }
         ...
         next_file = next_file->next )
   {
...
#ifdef READ_NUM_SLICES_FIRST
    {
      ...
      do
      {
         ...
         if ( read_io_block(iobuf,&item_header) != 0 )
            break;
      } while ( item_header.type != IO_TYPE_SIMTEL_TEL_MONI );
      if ( item_header.type == IO_TYPE_SIMTEL_TEL_MONI )
      {
         tmp_moni.tel_id = 
            (item_header.ident & 0xff) |
            ((item_header.ident & 0x3f000000) >> 16);
         printf("Found monitor block for telescope ID %d\n",tmp_moni.tel_id);
         read_simtel_tel_monitor(iobuf,&tmp_moni);
    ...
    for (;;) /* Loop over all data in the input file */
    {
      ...
      if ( hsdata == NULL && !skip_run &&
           item_header.type > IO_TYPE_SIMTEL_RUNHEADER &&
           item_header.type < IO_TYPE_SIMTEL_RUNHEADER + 200 &&
           item_header.type != IO_TYPE_SIMTEL_XTRGMASK )
      {
         if ( item_header.type == IO_TYPE_SIMTEL_MCRUNHEADER )
         {
            MCRunHeader tmp_mc_run_header;
            rc = read_simtel_mcrunheader(iobuf,&tmp_mc_run_header);
            if ( verbose || rc != 0 )
               printf("read_simtel_mcrunheader(), rc = %d\n",rc);
            if ( showdata )
               print_simtel_mcrunheader(iobuf);
           ...
           (dst_level >= 10 && dst_level <= 13) || dst_level >= 100 ) /* DST-type data extraction */
      {
         ...
         {
            if ( (int) item_header.type != IO_TYPE_SIMTEL_EVENT &&
                 (int) item_header.type != IO_TYPE_MC_TELARRAY )
            {
               if ( no_mc_data && 
                    ( (int) item_header.type == IO_TYPE_SIMTEL_MC_EVENT ||
                      (int) item_header.type == IO_TYPE_SIMTEL_MC_SHOWER ||
                      (int) item_header.type != IO_TYPE_SIMTEL_MC_PE_SUM ) )
         ...
         else if ( (dst_level%10) >= 2 || dst_level == 11 || dst_level == 10 || dst_level >= 101 ) /* MC data only for triggered/selected events */
         {
                 ...
                 (int) item_header.type != IO_TYPE_MC_TELARRAY )
            {
               if ( (dst_level%10) >= 3 && nrun > 1 && 
                    ( (int) item_header.type == IO_TYPE_SIMTEL_CAMSETTINGS ||
                      (int) item_header.type == IO_TYPE_SIMTEL_CAMORGAN ||
                      (int) item_header.type == IO_TYPE_SIMTEL_PIXELSET ||
                      (int) item_header.type == IO_TYPE_SIMTEL_TEL_MONI || 
                      (int) item_header.type == IO_TYPE_SIMTEL_LASCAL) )
      ...
      switch ( (int) item_header.type )
      {
         ...
         case IO_TYPE_SIMTEL_RUNHEADER:
            ...
            fflush(stdout);
            if ( (rc = read_simtel_runheader(iobuf,&hsdata->run_header)) < 0 )
            {
            ...
            if ( verbose || rc != 0 )
               printf("read_simtel_runheader(), rc = %d (run %d)\n",rc,hsdata->run_header.run);
            fprintf(stderr,"\nStarting run %d\n",hsdata->run_header.run);
            if ( showdata )
               print_simtel_runheader(iobuf);
            if ( user_ana )
         ...
         /* =================================================== */
         case IO_TYPE_SIMTEL_MCRUNHEADER:
            if ( skip_run )
               continue;
            rc = read_simtel_mcrunheader(iobuf,&hsdata->mc_run_header);
            if ( verbose || rc != 0 )
               printf("read_simtel_mcrunheader(), rc = %d\n",rc);
            if ( showdata )
               print_simtel_mcrunheader(iobuf);
         ...
         /* =================================================== */
         case IO_TYPE_HISTORY: /* How sim_hessarray was run and how it was configured. */
            if ( showhistory )
               list_history(iobuf,NULL);
            break;

         /* =================================================== */
         case IO_TYPE_METAPARAM:
            if ( showhistory )
               print_metaparam(iobuf);
            break;

         /* =================================================== */
         case IO_TYPE_SIMTEL_CAMSETTINGS:
            ...
            rc = read_simtel_camsettings(iobuf,&hsdata->camera_set[itel]);
            if ( verbose || rc != 0 )
               printf("read_simtel_camsettings(), rc = %d (tel. ID=%d, itel=%d)\n",rc,tel_id,itel);
            if ( showdata )
               print_simtel_camsettings(iobuf);
            ttype = 0;
            if ( reco_flag > 0 )
            {
               ttype = which_telescope_type(&hsdata->camera_set[itel]);
               ...
               if ( disabled_list_fname != NULL && ! disabled_list_done )
               {
                  if ( read_disabled_pixels_list(disabled_list_fname,manual_pixel_disabled) )
                  {
                     printf("Manual setting of disabled pixels may be incomplete or bad.\n");
                  }
                  printf("Done with manual setting of disabled pixels.\n");
                  disabled_list_done = 1;
               }
               if ( disabled_list_done && manual_pixel_disabled[itel] != NULL )
               {
                  // printf("Using manual list of disabled pixels for telescope %d for now.\n", tel_id);
                  memcpy(&hsdata->pixel_disabled[itel],manual_pixel_disabled[itel],sizeof(hsdata->pixel_disabled[itel]));
               }
               set_disabled_pixels(hsdata,itel,broken_pixels_fraction);
            }
            if ( num_onlytype > 0 )
            {
               if ( ttype <= 0 && reco_flag <= 0 ) /* type not yet checked */
                  ttype = which_telescope_type(&hsdata->camera_set[itel]);
               size_t itt;
               int type_ok = 0;
               for ( itt=0; itt<num_onlytype; itt++ )
               {
                  if ( only_type[itt] == ttype )
                  {
                     type_ok = 1;
                     break;
                  }
               ...
               if ( ! type_ok )
                  hsdata->camera_set[itel].num_mirrors = -3;
            }
            if ( num_only > 0 )
            {
         ...
         case IO_TYPE_SIMTEL_CAMORGAN:
            ...
            rc = read_simtel_camorgan(iobuf,&hsdata->camera_org[itel]);
            if ( verbose || rc != 0 )
               printf("read_simtel_camorgan(), rc = %d (tel. ID=%d, itel=%d)\n",rc,tel_id,itel);
            if ( showdata )
               print_simtel_camorgan(iobuf);
            break;

         /* =================================================== */
         case IO_TYPE_SIMTEL_PIXELSET:
            tel_id = item_header.ident; // Telescope ID is in the header
            if ( (itel = find_tel_idx(tel_id)) < 0 )
            {
               char msg[256];
            ...
            rc = read_simtel_pixelset(iobuf,&hsdata->pixel_set[itel]);
            if ( verbose || rc != 0 )
               printf("read_simtel_pixelset(), rc = %d (tel. ID=%d, itel=%d)\n",rc,tel_id,itel);
            if ( showdata )
               print_simtel_pixelset(iobuf);
            break;

         /* =================================================== */
         case IO_TYPE_SIMTEL_PIXELDISABLE:
            tel_id = item_header.ident; // Telescope ID is in the header
            if ( (itel = find_tel_idx(tel_id)) < 0 )
            {
               char msg[256];
            ...
            rc = read_simtel_pixeldis(iobuf,&hsdata->pixel_disabled[itel]);
            if ( verbose || rc != 0 )
               printf("read_simtel_pixeldis(), rc = %d (tel. ID=%d, itel=%d)\n",rc,tel_id,itel);
            if ( showdata )
               print_simtel_pixeldis(iobuf);
            if ( disabled_list_fname != NULL && ! disabled_list_done )
            {
               if ( read_disabled_pixels_list(disabled_list_fname,manual_pixel_disabled) )
               {
            ...
            if ( disabled_list_done && manual_pixel_disabled[itel] != NULL )
            {
               // printf("Using manual list of disabled pixels for telescope %d.\n", tel_id);
               memcpy(&hsdata->pixel_disabled[itel],manual_pixel_disabled[itel],sizeof(hsdata->pixel_disabled[itel]));
            }
            set_disabled_pixels(hsdata,itel,broken_pixels_fraction);
            if ( iobuf->output_file != NULL && broken_pixels_fraction > 0. )
            {
               /* Output was requested and broken pixels got added.
                  Write the new list of broken pixels after the original one. */
               if ( write_simtel_pixeldis(iobuf,&hsdata->pixel_disabled[itel]) < 0 )
                  printf("Writing new pixel disable block failed.\n");
            }
            break;

         /* =================================================== */
         case IO_TYPE_SIMTEL_CAMSOFTSET:
            tel_id = item_header.ident; // Telescope ID is in the header
            if ( (itel = find_tel_idx(tel_id)) < 0 )
            {
               char msg[256];
            ...
            rc = read_simtel_camsoftset(iobuf,&hsdata->cam_soft_set[itel]);
            if ( verbose || rc != 0 )
               printf("read_simtel_camsoftset(), rc = %d (tel. ID=%d, itel=%d)\n",rc,tel_id,itel);
         ...
         /* =================================================== */
         case IO_TYPE_SIMTEL_POINTINGCOR:
            tel_id = item_header.ident; // Telescope ID is in the header
            if ( (itel = find_tel_idx(tel_id)) < 0 )
            {
               char msg[256];
            ...
            rc = read_simtel_pointingcor(iobuf,&hsdata->point_cor[itel]);
            if ( verbose || rc != 0 )
               printf("read_simtel_pointingcor(), rc = %d (tel. ID=%d, itel=%d)\n",rc,tel_id,itel);
            /* No print function available */
            if ( showdata )
               print_simtel_pointingcor(iobuf);
            break;

         /* =================================================== */
         case IO_TYPE_SIMTEL_TRACKSET:
            tel_id = item_header.ident; // Telescope ID is in the header
            if ( (itel = find_tel_idx(tel_id)) < 0 )
            {
               char msg[256];
            ...
            rc = read_simtel_trackset(iobuf,&hsdata->tracking_set[itel]);
            if ( verbose || rc != 0 )
               printf("read_simtel_trackset(), rc = %d (tel. ID=%d, itel=%d)\n",rc,tel_id,itel);
            /* No print function available */
            if ( showdata )
               print_simtel_trackset(iobuf);
            break;

         /* =================================================== */
         case IO_TYPE_SIMTEL_EVENT:
            if ( skip_run || skip_shower )
               continue;
            ...
            rc = read_simtel_event(iobuf,&hsdata->event,-1);
            if ( verbose || rc != 0 )
               printf("read_simtel_event(), rc = %d\n",rc);

            if ( only_events.from != 0 || only_events.to != 0 )
            {
               if ( !is_in_range(hsdata->mc_event.event, &only_events) )
               {
                  if ( verbose )
                     printf("Skip event %d because not in selected event range.\n", hsdata->mc_event.event);
                  continue;
               }
            }
            ...
            if ( reco_verbose_level > 1 )
               printf("Event %d has %d telescopes with data (%ju with raw data, %ju with images).\n",
                  hsdata->mc_event.event, ntel_trg, num_tel_raw, num_tel_img);
            ...
            if ( max_tel_trg > 0 && ntel_trg > max_tel_trg )
               continue;
            if ( num_tel_img < min_tel_img )
               continue;
            if ( max_tel_img > 0 && num_tel_img > max_tel_img )
               continue;

            /* If a specific telescope is required, check for its presence. */
            if ( req_tel >= 0 )
            {
               int have_req_tel = 0;
               for (itel=0; itel<hsdata->run_header.ntel; itel++)
               {
                  if ( hsdata->event.teldata[itel].known )
                  {
                     if ( hsdata->event.teldata[itel].tel_id == req_tel )
                     {
                        have_req_tel = 1;
                        break;
                     }
            ...
            if ( reco_flag >= 5 ) 
               for ( itel=0; itel<hsdata->run_header.ntel; itel++ )
                  ...
                  {
                     if ( reco_verbose_level > 1 )
                        printf("Plot original camera image(s) for telescope %d in event %d\n",
                           hsdata->event.teldata[itel].tel_id, hsdata->mc_event.event);
                     if ( show_true_pe &&
                          ( hsdata->mc_event.mc_pe_list[itel].npe > 0 ||
                            ( hsdata->mc_event.mc_pesum.num_pe[itel] > 0 &&
            ...
            {
               if ( reco_verbose_level > 1 )
                  printf("Reconstruct images and shower in event %d\n",
                     hsdata->mc_event.event);
            ...
            {
               if ( reco_verbose_level > 1 )
                  printf("Call user analysis in event %d\n",
                     hsdata->mc_event.event);
            ...
            for (itel=0; itel<hsdata->run_header.ntel; itel++)
            {
               ...
               {
                  if ( reco_verbose_level > 1 )
                     printf("Plot current camera image(s) for telescope %d in event %d\n",
                        hsdata->event.teldata[itel].tel_id, hsdata->mc_event.event);
                  if ( show_true_pe &&
                          ( hsdata->mc_event.mc_pe_list[itel].npe > 0 ||
                            ( hsdata->mc_event.mc_pesum.num_pe[itel] > 0 &&
                  ...
                  dst_level >= 100 )
            {
               if ( dst_level != 100 && !no_mc_data )
               {
                  ...
                  {
                     if ( write_simtel_mc_shower(iobuf,&hsdata->mc_shower) < 0 )
                     {
                  ...
                  {
                     if ( write_simtel_mc_event(iobuf,&hsdata->mc_event) < 0 )
                     {
                  ...
                  if ( mc_pe_sum_stored == 0 && (dst_level%10) == 2 &&
                       last_mc_pe_sum_id == item_header.ident )
                  {
                     if ( write_simtel_mc_pe_sum(iobuf,&hsdata->mc_event.mc_pesum) < 0 )
                     {
               ...
               if ( dst_level == 0 )
                  rc = write_simtel_event(iobuf,&hsdata->event,-1 ^ RAWDATA_FLAG );
               else if ( dst_level == 10 || cleaning > 0 || dst_level >= 100 )
                  rc = write_simtel_event(iobuf,&hsdata->event,-1);
               else
                  rc = write_simtel_event(iobuf,&hsdata->event,
         ...
         /* =================================================== */
         case IO_TYPE_SIMTEL_CALIBEVENT:
            if ( skip_run )
               continue;
         {
            int type = -1;
            rc = read_simtel_calib_event(iobuf,&hsdata->event,-1,&type);
            if ( verbose || rc != 0 )
               printf("read_simtel_calib_event(), rc = %d, type=%d\n",rc,type);
            if ( showdata )
               print_simtel_calib_event(iobuf);
            for (itel=0; itel<hsdata->run_header.ntel; itel++)
               if ( hsdata->event.teldata[itel].known )
         ...
         /* =================================================== */
         case IO_TYPE_SIMTEL_CALIB_PE:
            if ( skip_run )
               continue;
         {
            int type = -1;
            rc = read_simtel_calib_pe(iobuf,&hsdata->mc_event,&type);
            if ( verbose || rc != 0 )
               printf("read_simtel_calib_pe(), rc = %d, type=%d\n",rc,type);
            if ( showdata )
               print_simtel_calib_pe(iobuf);
         }
            break;
         ...
         case IO_TYPE_SIMTEL_MC_SHOWER:
            if ( skip_run )
               continue;
            skip_shower = 0;
            if ( only_showers.from != 0 || only_showers.to != 0 )
            {
               if ( !is_in_range(item_header.ident, &only_showers) )
               {
                  skip_shower = 1;
               }
            }
            rc = read_simtel_mc_shower(iobuf,&hsdata->mc_shower);
            if ( verbose || rc != 0 )
               printf("read_simtel_mc_shower(), rc = %d (shower %d)\n",
                  rc, hsdata->mc_shower.shower_num);
            if ( showdata )
               print_simtel_mc_shower(iobuf);
            if ( user_ana )
               do_user_ana(hsdata,item_header.type,0);
            break;

         /* =================================================== */
         case IO_TYPE_SIMTEL_MC_EVENT:
#ifdef CHECK_MISSING_PE_LIST
            got_pe_list = 0;
#endif
            if ( skip_run || skip_shower )
               continue;
            rc = read_simtel_mc_event(iobuf,&hsdata->mc_event);
            if ( verbose || rc != 0 )
               printf("read_simtel_mc_event(), rc = %d (event %d, shower %d)\n",
                  rc, hsdata->mc_event.event, hsdata->mc_event.shower_num);
            if ( showdata )
               print_simtel_mc_event(iobuf);
            if ( user_ana )
               do_user_ana(hsdata,item_header.type,0);
            if ( (dst_level%10) >= 2 )
         ...
         case IO_TYPE_MC_TELARRAY:
            ...
            {
               rc = read_simtel_mc_phot(iobuf,&hsdata->mc_event);
               if ( verbose || rc != 0 )
                  printf("read_simtel_mc_phot(), rc = %d\n",rc);
            }
            if ( showdata )
               print_simtel_mc_phot(iobuf);
            break;
         ...
         case IO_TYPE_MC_ATMPROF:
            rc = read_atmprof(iobuf,get_common_atmprof());
            if ( verbose || rc != 0 )
               printf("read_atmprof(), rc = %d\n",rc);
            if ( showdata )
               print_atmprof(iobuf);
            break;

         /* =================================================== */
         case IO_TYPE_SIMTEL_MC_PE_SUM:
            if ( skip_run || skip_shower )
               continue;
            rc = read_simtel_mc_pe_sum(iobuf,&hsdata->mc_event.mc_pesum);
            if ( verbose || rc != 0 )
               printf("read_simtel_mc_pe_sum(), rc = %d\n",rc);
            if ( showdata )
               print_simtel_mc_pe_sum(iobuf);
            break;

         /* =================================================== */
         case IO_TYPE_SIMTEL_TEL_MONI:
            ...
            rc = read_simtel_tel_monitor(iobuf,&hsdata->tel_moni[itel]);
            if ( verbose || rc != 0 )
               printf("read_simtel_tel_monitor(), rc = %d (tel. ID=%d, itel=%d)\n",
                  rc, tel_id, itel);
            if ( showdata )
               print_simtel_tel_monitor(iobuf);
            break;

         /* =================================================== */
         case IO_TYPE_SIMTEL_MC_PIXMON:
            tel_id = item_header.ident;
            if ( (itel = find_tel_idx(tel_id)) < 0 )
            {
               char msg[256];
               snprintf(msg,sizeof(msg)-1,
                  "MC pixel monitor block for unknown telescope %d.", tel_id);
               Warning(msg);
               exit(1);
            }
            rc = read_simtel_mc_pixel_moni(iobuf,&hsdata->mcpixmon[itel]);
            if ( verbose || rc != 0 )
               printf("read_simtel_mc_pixel_moni(), rc = %d (tel. ID=%d, itel=%d)\n",
                  rc, tel_id, itel);
            if ( showdata )
               print_simtel_mc_pixel_moni(iobuf);
            break;

         /* =================================================== */
         case IO_TYPE_SIMTEL_LASCAL:
            tel_id = item_header.ident; // Telescope ID is in the header
            if ( (itel = find_tel_idx(tel_id)) < 0 )
            {
               char msg[256];
            ...
            rc = read_simtel_laser_calib(iobuf,&hsdata->tel_lascal[itel]);
            if ( verbose || rc != 0 )
               printf("read_simtel_laser_calib(), rc = %d (tel. ID=%d, itel=%d)\n",
                  rc, tel_id, itel);
            if ( showdata )
               print_simtel_laser_calib(iobuf);
            ...
            if ( calerror > 0.0 )
            {
               ...
               if ( iobuf->output_file != NULL )
               {
                  /* Output was requested and the calibration coefficients got
                     deteriorated. Write the modified coefficients after the original ones. */
                  rc = write_simtel_laser_calib(iobuf,&hsdata->tel_lascal[itel]);
                  if ( rc < 0 )
                     printf("Writing modified laser calibration failed.\n");
               }
            }
            break;

         /* =================================================== */
         case IO_TYPE_SIMTEL_RUNSTAT:
            if ( skip_run )
               continue;
            rc = read_simtel_run_stat(iobuf,&hsdata->run_stat);
            if ( verbose || rc != 0 )
               printf("read_simtel_run_stat(), rc = %d\n",rc);
            if ( showdata )
               print_simtel_run_stat(iobuf);
            if ( user_ana )
               do_user_ana(hsdata,item_header.type,0);
            break;

         /* =================================================== */
         case IO_TYPE_SIMTEL_MC_RUNSTAT:
            if ( skip_run )
               continue;
            rc = read_simtel_mc_run_stat(iobuf,&hsdata->mc_run_stat);
            if ( verbose || rc != 0 )
               printf("read_simtel_mc_run_stat(), rc = %d\n",rc);
            if ( showdata )
               print_simtel_mc_run_stat(iobuf);
            if ( user_ana )
               do_user_ana(hsdata,item_header.type,0);
            if ( (dst_level%10) >= 2 )
         ...
         default:
            ...
            if ( item_header.type == 2009 )
            {
               if ( showdata )
               {
                  ...
                  print_simtel_centralevent(iobuf);
               }
               rc = read_simtel_centralevent(iobuf,&hsdata->event.central);
               if ( verbose || rc != 0 )
                  printf("read_simtel_centralevent(), rc = %d\n",rc);
               continue;
            }
            else if ( item_header.type > 2000 && item_header.type < 10000 )
            {
               if ( item_header.type%1000 >= 100 && item_header.type%1000 <= 199 )
               {
                  ...
                  rc = read_simtel_trackevent(iobuf,&hsdata->event.trackdata[itel]);
                  if ( verbose || rc != 0 )
                     printf("read_simtel_trackevent(), rc = %d\n",rc);
                  continue;
               }
               else if ( item_header.type%1000 >= 200 && item_header.type%1000 <= 299 )
               {
                  int tid = (item_header.type - IO_TYPE_SIMTEL_TELEVENT)%100 +
                            100 * ((item_header.type - IO_TYPE_SIMTEL_TELEVENT)/1000);
                  if ( showdata )
                  {
                     printf("Stand-alone telescope event data block for telescope %d:\n", tid);
                     print_simtel_televent(iobuf);
                  }
                  if ( (itel = find_tel_idx(tid)) < 0 || itel >= H_MAX_TEL )
                  {
                  ...
                  rc = read_simtel_televent(iobuf,&hsdata->event.teldata[itel],-1);
                  if ( verbose || rc != 0 )
                     printf("read_simtel_televent(), rc = %d\n",rc);
   ...
   {
      if ( nev_tot > 0 )
         printf("Total: %d of %d events triggered\n", ntrg_tot, nev_tot);
      else
         printf("Total: no events.\n");
   }
