...
Copyright (C) 2003, 2004, 2005, 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2022, 2023  Konrad Bernloehr
...
/** Set up pixels to be ignored (regarded as zero amplitude) in the analysis
    if they either 
    a) are reported as having HV disabled (no signal) in the input data stream or in a custom list, 
    b) the camera active radius gets clipped and the pixel is outside, or
    c) they are randomly chosen to be ignored.
   ...
   int flag_amp_tm, double clip_amp)
{
   ...
   for (i=0; i<npix; i++)
   {
      ...
      if ( !significant ) 
         ...
         npe = npe_hg;
#if ( H_MAX_GAINS >= 2 )
      else if ( raw->num_gains >= 2 && lg_known )
         npe = npe_lg;
#endif
      else if ( hg_known )
         npe = npe_hg;
...
static int nb_peak_integration(AllHessData *hsdata, int lwt, int itel, int nsum, int nbefore, int *sigamp /* unused */);

static int nb_peak_integration(AllHessData *hsdata, int lwt, int itel, int nsum, int nbefore, UNUSED_PAR2(int *,sigamp) )
{
 ...
 *                  as long as it fits into the available data range.
 *                  Note: for multiple gains, this may result in identical integration regions (depending on signal).
 *  @param sigamp (not used, just for similarity with other integration functions)
...
static int nb_fc_shaped_peak_integration(AllHessData *hsdata, int itel, int nsum, int nbefore, int *sigamp /* unused */, int psopt, int ithr);

static int nb_fc_shaped_peak_integration(AllHessData *hsdata, int itel, int nsum, int nbefore, UNUSED_PAR2(int *,sigamp), int psopt, int ithr)
{
...
// printf("###> %f:  %f  %f  %f  %f\n", traw[ibin/4]+ibin%4, (adc[ibin/4]-1000.), shaped0[ibin], shaped1[ibin], shaped[ibin]);
      ...
      {
          integration_correction[itel][igain] = 1.0;
          if ( igain == 0 )
            printf("No integration correction for fixed window integration in telescope #%d (ID=%d).\n",
              itel, raw->tel_id);
      }
      ...
      if ( integrator > 1 )
      {
         ...
         // printf("   st = %f, noff = %d, ipeak = %d, nbins = %d, lrefshape = %d\n", st, noff, ipeak, nbins, ps->lrefshape);
      }
   }
   return 0;
}
...
int clean_raw_data (AllHessData *hsdata, int itel, int clean_flag, 
   int tcl /* unused */, int tch /* unused */, struct user_parameters *up /* unused */);
   
int clean_raw_data (AllHessData *hsdata, int itel, int clean_flag, 
   UNUSED_PAR2(int, tcl), UNUSED_PAR2(int, tch), UNUSED_PAR2(struct user_parameters *,up))
{
...
static int fill_pixel_trg_stats(AllHessData *hsdata, int itel, int tel_type, struct user_parameters *up)
{
   ...
   if ( npix > H_MAX_PIX )
      return -1;
   int has_triggered[H_MAX_PIX];
   double npe[H_MAX_PIX];
#if 0
   int nsect = hdata->camera_org[itel].num_sectors;
   if ( nsect > H_MAX_SECTORS )
      return -1;
   int ntrgsect = teldata->num_list_trgsect;
   int sect_has_triggered[H_MAX_SECTORS];
   double npesum[H_MAX_SECTORS];
#endif
   int ht = 100000 * tel_type;
   int hist_base = ht + 21000;
   double E_true = hsdata->mc_shower.energy;    /**< true energy [TeV] */
   double ewt = (E_true > 1e-6) ?  pow(E_true, up[0].d.d_sp_idx) : 1.0;   /**< Event for desired spectral slope */
   ...
   {
      int itrg = teldata->trigger_pixels.pixel_list[i];
      if ( itrg >= 0 && itrg < npix )
         has_triggered[itrg] = 1;
   }
   
#if 0
   for ( i=0; i<ntrgsect; i++ )
   {
      itrgsect = teldata->list_trgsect[i];
      if ( itrgsect >= 0 && itrgsect < ntrgsect )
         sect_has_triggered[itrgsect] = 1;
   }
#endif
   ...
   for ( i=0; i<npix; i++ )
   {
      ...
      npe[i] = calibrate_pixel_amplitude(hsdata,itel,i,0,-1,0);
      fill_histogram_by_ident(hist_base+1,npe[i],0.,ewt);
      if ( has_triggered[i] )
         fill_histogram_by_ident(hist_base+2,npe[i],0.,ewt);
   }

#if 0
   for ( j=0; j<nsect; j++ )
   {
      for ( i=0; i< .... // FIXME: code incomplete
   }
#endif 
      ...
      if ( have_raw_data )
      {
         for (itel=0; itel<hsdata->run_header.ntel; itel++)
         {
                 ...
                 hsdata->event.teldata[itel].raw->known )
            {
               ...
               /* Pixel and trigger group trigger efficiency evaluation on request only */
               if ( up->i.pixstat )
               {
                  fill_pixel_trg_stats(hsdata, itel, tel_type, up);
               }
