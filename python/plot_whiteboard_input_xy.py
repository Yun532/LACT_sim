...
def read_xy(path: Path, event_id: int | None, telescope_id: int | None) -> tuple[np.ndarray, np.ndarray]:
    ...
    with path.open(newline="") as handle:
        ...
        if "input_x_m" not in fields or "input_y_m" not in fields:
            raise SystemExit(
                f"{path} does not contain input_x_m/input_y_m. "
                "Re-run run_corsika_trace with the diagnostic whiteboard CSV columns enabled."
            )
        for row in reader:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="比较命中白板光子的输入平面 x/y 分布。"
    )
    parser.add_argument("csv", nargs="+", help="白板 hit CSV 文件")
    parser.add_argument("--label", action="append", help="图例标签；每个 CSV 重复一次")
    parser.add_argument("--event-id", type=int)
    parser.add_argument("--telescope-id", type=int, help="从 0 开始的 telescope id")
    ...
    title = "Whiteboard-hit photons: input-plane x/y distribution"
    if subtitle:
        title += " (" + ", ".join(subtitle) + ")"
    fig.suptitle(title)
    fig.tight_layout()
