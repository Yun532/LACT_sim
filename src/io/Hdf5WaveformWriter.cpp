...
struct ParentComponentWaveformSample {
    std::int32_t image_index = 0;
    std::int32_t time_bin = 0;
    std::int32_t pixel_id = 0;
    std::int16_t component_id = 5;
    std::int32_t photon_count = 0;
    float pe = 0.0f;
};
...
void writeParentComponentSamples(
    hid_t group,
    const std::vector<ParentComponentWaveformSample>& samples)
{
    hid_t type = H5Tcreate(
        H5T_COMPOUND, sizeof(ParentComponentWaveformSample));
    H5Tinsert(type, "image_index",
              HOFFSET(ParentComponentWaveformSample, image_index),
              H5T_NATIVE_INT32);
    H5Tinsert(type, "time_bin",
              HOFFSET(ParentComponentWaveformSample, time_bin),
              H5T_NATIVE_INT32);
    H5Tinsert(type, "pixel_id",
              HOFFSET(ParentComponentWaveformSample, pixel_id),
              H5T_NATIVE_INT32);
    H5Tinsert(type, "component_id",
              HOFFSET(ParentComponentWaveformSample, component_id),
              H5T_NATIVE_INT16);
    H5Tinsert(type, "photon_count",
              HOFFSET(ParentComponentWaveformSample, photon_count),
              H5T_NATIVE_INT32);
    H5Tinsert(type, "pe",
              HOFFSET(ParentComponentWaveformSample, pe),
              H5T_NATIVE_FLOAT);
    hsize_t dims[1] = {samples.size()};
    hid_t space = H5Screate_simple(1, dims, nullptr);
    hid_t dataset = H5Dcreate2(
        group, "waveform", type, space,
        H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    if (!samples.empty()) {
        check(H5Dwrite(dataset, type, H5S_ALL, H5S_ALL,
                       H5P_DEFAULT, samples.data()),
              "dataset parent_components/waveform");
    }
    H5Dclose(dataset);
    H5Sclose(space);
    H5Tclose(type);
...
void writeHdf5Waveforms(
    ...
    const std::vector<RawParentComponentWaveformHit>& raw_parent_hits)
{
    ...
    if (output.hdf5_write_parent_component_waveforms &&
        waveform.source == "pe") {
        using ParentKey =
            std::tuple<std::int32_t, std::int32_t, std::int32_t, std::int16_t>;
        std::map<ParentKey, ParentComponentWaveformSample> samples_by_key;
        for (const auto& hit : raw_parent_hits) {
            const auto image = image_row_by_key.find(
                {hit.event_id, hit.telescope_id});
            if (image == image_row_by_key.end() ||
                pixel_to_col.find(hit.pixel_id) == pixel_to_col.end()) {
                continue;
            }
            const std::size_t row = image->second;
            const int bin = binForTime(
                waveform, hit.time_ns - reference_time_ns[row]);
            if (bin < 0) {
                continue;
            }
            const std::int32_t image_index = images[row].image_index;
            const std::int32_t time_bin = static_cast<std::int32_t>(bin);
            const std::int32_t pixel_id =
                static_cast<std::int32_t>(hit.pixel_id);
            const std::int16_t component_id =
                static_cast<std::int16_t>(hit.component_id);
            const ParentKey key{
                image_index, time_bin, pixel_id, component_id};
            auto& sample = samples_by_key[key];
            sample.image_index = image_index;
            sample.time_bin = time_bin;
            sample.pixel_id = pixel_id;
            sample.component_id = component_id;
            sample.photon_count +=
                static_cast<std::int32_t>(hit.photon_count);
            sample.pe += static_cast<float>(hit.pe);
        }

        std::vector<ParentComponentWaveformSample> parent_samples;
        parent_samples.reserve(samples_by_key.size());
        for (const auto& item : samples_by_key) {
            parent_samples.push_back(item.second);
        }

        hid_t parent_group = H5Gcreate2(
            file, "parent_components",
            H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
        writeStringAttribute(parent_group, "component_ids",
        ...
        writePlain1D(parent_group, "pixel_id_axis",
                     H5T_NATIVE_INT32, pixel_id_axis);
        writePlain1D(parent_group, "time_edges_ns",
                     H5T_NATIVE_DOUBLE, time_edges);
        writePlain1D(parent_group, "time_centers_ns",
                     H5T_NATIVE_DOUBLE, time_centers);
        writePlain1D(parent_group, "reference_time_ns",
                     H5T_NATIVE_DOUBLE, reference_time_ns);
        writeParentComponentSamples(parent_group, parent_samples);
