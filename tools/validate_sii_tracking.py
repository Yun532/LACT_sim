...
from sii_observation import tracking_geometry, simulate_array_waveforms, align_waveforms, correlate_blocks
...
def main():
    ...
    parser.add_argument('--output', type=Path, default=ROOT/'validation/sii_tracking')
    ...
    rows, result_rows, checks, epoch_rows = [], [], [], []
    ...
    for epoch_index, elapsed in enumerate(epochs):
        ...
        train = series['calibration'][:, 0]/(scale*truth)
    ...
    for baseline, group in results.groupby('baseline'):
        line = axes[1].errorbar(group.elapsed_s/3600, group.tracked_power/group.truth,
            yerr=1.96*group.tracked_total_sem/group.truth, marker='o', ms=3, label=baseline+' tracked')
        axes[1].errorbar(group.elapsed_s/3600, group.initial_delay_power/group.truth,
            yerr=1.96*group.initial_delay_total_sem/group.truth, fmt='--',
            color=line[0].get_color(), label=baseline+' initial delay')
    axes[1].axhline(1., color='k', lw=.7)
    axes[1].set(xlabel='Time from transit [h]', ylabel='Recovered / injected power',
    ...
    summary = dict(seed=args.seed, records=args.records, calibration_records=args.calibration_records,
        ...
        scope='Seven independent short-record epochs; not continuous six-hour ADC or full-array covariance. Fixed test coherence, not a stellar light curve.',
        ...
        star_rate_hz=star, background_rate_hz=background, coherence=gamma.tolist(),
